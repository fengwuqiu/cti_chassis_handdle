...
McuOta::McuOta(std::shared_ptr <rclcpp::Node> node_ptr, std::vector <std::string> parameters) :
    ...
    sp_total_crc = 0;
    fw_packet = {0};
    manager_pub = node_ptr->create_publisher<cti_fpga_serial_msgs::msg::UpdateInfo>("cti/fpga_serial/stmupdate", 20);
    manager_sub = node_ptr->create_subscription<cti_fpga_serial_msgs::msg::UpdateInfo>("cti/fpga_serial/stminfo", 5,
...
//recv
void McuOta::ros_recv_data_call_back(const cti_fpga_serial_msgs::msg::UpdateInfo::SharedPtr msg) 
{
...
void McuOta::send_data() 
{
    ...
    if (send_buf_size > 0) {
        auto msg = cti_fpga_serial_msgs::msg::UpdateInfo();
        msg.seq_num = seq_num++;
