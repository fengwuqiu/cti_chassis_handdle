...
McuOta::McuOta(std::shared_ptr<rclcpp::Node> node_ptr,
               std::vector<std::string> parameters)
    : m_node_ptr(node_ptr), input_parameters_(parameters) {
  ros_recv = {0};
  ros_send = {0};
  serial_frame = {0};
  serial_recv = {0};
  ...
  sp_total_crc = 0;
  fw_packet = {0};
  manager_pub = node_ptr->create_publisher<cti_chassis_msgs::msg::UpdateInfo>(
      "cti/fpga_serial/stmupdate", 20);
  manager_sub =
      node_ptr->create_subscription<cti_chassis_msgs::msg::UpdateInfo>(
          "cti/fpga_serial/stminfo", 5,
...
int McuOta::find_frame_header(int pos, char *found) {
  ...
  while (i < (serial_recv.cnt - FRAME_HEADER_SIZE)) {
    ...
    while (match_cnt < FRAME_HEADER_SIZE) {
      if (serial_recv.buf[RECEIVE_RING_ADD(pos, i)] !=
...
int McuOta::parse_recv_frame() {
  unsigned char tail =
      (recv_frame_queue.head + recv_frame_queue.cnt) % MAX_RECV_FRAME_QUEUE;
  ...
  parse_data_from_recv((unsigned char *)&frame->data,
                       (serial_recv.start + sizeof(serial_fixed_header_type)) %
                           RECEIVE_BUFFER_SIZE,
...
int McuOta::construct_serial_frame(serial_frame_type *frame, unsigned char cmd,
                                   void *data) {
  if (cmd > CMD_MAX) {
    RCLCPP_ERROR_STREAM(m_node_ptr->get_logger(),
                        "Invalid cmd type when init serial frame!!! " << cmd);
    return -1;
  }
  int length = get_cmd_data_length(cmd);
  if ((-1 == length) ||
      (length + sizeof(serial_fixed_header_type) > sizeof(serial_frame_type))) {
    RCLCPP_ERROR_STREAM(m_node_ptr->get_logger(),
                        "Oops, we got a cmd_data_length[cmd] > "
                        "sizeof(serial_frame.data),cmd_data_length: "
                            << length << " sizeof(serial_frame.data): "
                            << (int)sizeof(serial_frame_type) -
                                   (int)sizeof(serial_fixed_header_type)
...
int McuOta::pack_send_frame_buf_in_queue(serial_frame_type *frame,
  ...
  if (send_frame_queue.cnt >= MAX_SEND_FRAME_QUEUE) {
    RCLCPP_INFO_STREAM(
        m_node_ptr->get_logger(),
        "Send frame queue high level detected, cnt: " << send_frame_queue.cnt);
    return -1;
  }
  /*set resend param*/
  send_frame_queue.resend[(send_frame_queue.head + send_frame_queue.cnt) %
                          MAX_SEND_FRAME_QUEUE] = resend;
  /*add buf to the queue tail.*/
  send_buf =
      &(send_frame_queue.frame[(send_frame_queue.head + send_frame_queue.cnt) %
                               MAX_SEND_FRAME_QUEUE]);
...
void McuOta::check_cmd_overtime_clock() {
  ...
  if (m_node_ptr->get_clock()->now().seconds() - cmd_overtime.start_time >
      cmd_overtime.delay_time) {
    cmd_overtime.enable_check = false;
    ota_state->set_report_information(UPDATE_FAILED, firmware_upgrade_flag,
                                      "Wait cmd response overtime");
    firmware_upgrade_flag = FIRMWARE_ABORT_UPGRADE;
  }
}

void McuOta::send_update_status_cmd_once(unsigned char status,
...
int McuOta::init_firmware_upgrade_params(unsigned long size) {
  ...
  fw_packet.total_packets =
      fw_packet.firmware_size / fw_packet.packet_length + extra_packet;
  fw_packet.current_packet = extra_packet;
  fw_packet.actual_length =
      (fw_packet.current_packet == fw_packet.total_packets)
          ? (fw_packet.firmware_size % fw_packet.packet_length)
          : fw_packet.packet_length;
  fw_packet.read_offset = 0;
  sp_total_crc = 0;
  // test
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "fw_packet.firmware_size: " << fw_packet.firmware_size);
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "fw_packet.current_packet: " << fw_packet.current_packet);
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "fw_packet.total_packets: " << fw_packet.total_packets);
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "fw_packet.packet_length: " << fw_packet.packet_length);
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "fw_packet.actual_length: " << fw_packet.actual_length);
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "fw_packet.read_offset: " << fw_packet.read_offset);
  return 0;
}
...
int McuOta::init_firmware_upgrade() {
  ...
  if (-1 == (fd = open(update_option_.bin_file_name.c_str(), O_RDONLY))) {
    RCLCPP_ERROR_STREAM(m_node_ptr->get_logger(),
                        "Open  Error" << update_option_.bin_file_name);
    return fd;
  }
  ...
  RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                      "the total file crc is:" << total_crc);
  lseek(fd, 0L, SEEK_SET);
...
int McuOta::do_single_firmware_update(unsigned int state, OtaState *result) {
  ...
  switch (state) {
    case INFORM_UPDATE: {
      ...
      construct_serial_frame(&serial_frame, UPD_CMD_211, (void *)&upd_cmd_211);
      pack_send_frame_buf_in_queue(&serial_frame,
                                   (UPD_CMD_211 << 8) + MAX_CMD_RESEND_TIME);
    ...
    case SEND_MAIN_MODULE_ID: {
      ...
      pack_send_frame_buf_in_queue(&serial_frame,
                                   (UPD_CMD_212 << 8) + MAX_CMD_RESEND_TIME);
    ...
    case FIRMWARE_NORMAL: {
      ...
      pack_send_frame_buf_in_queue(&serial_frame,
                                   (UPD_CMD_209 << 8) + MAX_CMD_RESEND_TIME);
    ...
    case FIRMWARE_CHECK_VERSION_OK: {
      RCLCPP_INFO_STREAM(
          m_node_ptr->get_logger(),
          "Detect upgrade firmware file, begin to upgrade firmware.");
      if (-1 != init_firmware_upgrade()) {
        ...
        if (5 == input_parameters_.size()) {
          upd_cmd_200.key[0] = (unsigned char)(atoi(
              input_parameters_[4].c_str()));  //新增key  std::stoi(str)
        }
        construct_serial_frame(&serial_frame, UPD_CMD_200,
                               (void *)&upd_cmd_200);
        pack_send_frame_buf_in_queue(&serial_frame,
                                     (UPD_CMD_200 << 8) + MAX_CMD_RESEND_TIME);
        set_cmd_overtime_clock(UPD_CMD_200, MAX_CMD_OVERTIME_VALUE);
        firmware_upgrade_flag = FIRMWARE_PRE_UPDATE;
        RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                           "In state FIRMWARE_NORMAL send cmd_200.");
      } else {
        firmware_upgrade_flag = FIRMWARE_ABORT_UPGRADE;
        ota_state->set_report_information(UPDATE_FAILED,
                                          FIRMWARE_CHECK_VERSION_OK,
                                          "Open upgrade file error.");
      }
    } break;
    ...
    case FIRMWARE_ABORT_UPGRADE: {
      ...
      RCLCPP_ERROR_STREAM(m_node_ptr->get_logger(),
                          "Firmware upgrade failed, abort the module upgrade.");
    } break;
    case FIRMWARE_UPGRADE_FINISHED: {
      ...
      ota_state->set_report_information(
          UPDATE_SUCCESSED, FIRMWARE_UPGRADE_FINISHED, "Update file finished.");
      manage_update_state = RECORD_STATUS;
    } break;
    default:
      break;
...
void McuOta::send_update_status_cmd(unsigned char status,
                                    unsigned char target_id) {
  upd_cmd_211_type upd_cmd_211 = {0};
  set_upd_info(&upd_cmd_211.upd_info, target_id);
  upd_cmd_211.status = status;  // upgrade notice to module.
...
int McuOta::manage_module_updates(int option, int main_option) {
  ...
  switch (manage_update_state) {
    case INIT_UPDATE_LIST: {
      ...
      ota_state->upgrade_report.main_module_name =
          update_option_.main_module_name;
      ...
      ota_state->set_report_information(UPDATE_START, firmware_upgrade_flag,
                                        "update start.");
    ...
    case RECORD_STATUS: {
      ...
      if (true ==
          ota_state->upgrade_report.motor_driver_main_bus_comm_disable) {
        usleep(20000);
        for (int i = MODULE_MOTOR_DRIVER_FRONT_LEFT;
             i <= MODULE_MOTOR_DRIVER_BACK_RIGHT; i++) {
      ...
      if (true == ota_state->upgrade_report.motor_driver_aux_bus_comm_disable) {
        usleep(20000);
        for (int i = MODULE_MOTOR_TURN_FRONT; i <= MODULE_MOTOR_BREAK_BACK;
             i++) {
          send_update_status_cmd_once(ret, i);  // 2 or 3
          usleep(20000);
        }
...
// recv
void McuOta::ros_recv_data_call_back(
  ...
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "recv a ros package: seq_num: "
                         << msg->seq_num << " data_size: " << msg->data.size());
...
int McuOta::ros_buffer_read(unsigned char *dest, int cnt) {
  ...
  int actual_len =
      (ros_recv.end + ROS_COMM_BUF_SIZE - ros_recv.start) % ROS_COMM_BUF_SIZE;
...
int McuOta::query_serial_data() {
  ...
  if (remain_buffer_cnt < LOW_RECEIVE_BUFFER_THRESHOLD) {
    RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                        "Low recv buffer level!****cleaning recv buffer!");
    init_recv_buffer();
    ret = 0;
    return ret;
  }
  ...
  if (pos >= serial_recv.start) {
    ...
    ret = ros_buffer_read(serial_recv.buf + pos,
...
void McuOta::find_serial_frame() {
  ...
  RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                      "Finding a frame from total "
                          << serial_recv.cnt << " buffer chars, new recvived "
                          << ret << " chars, start pos:" << serial_recv.start);
  while (serial_recv.cnt) {
    switch (serial_recv.state) {
      case RECV_STATE_FINDING_HEADER: {
        RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                            "In state RECV_STATE_FINDING_HEADER");
        ...
        if (found_header) {
          serial_recv.start =
              RECEIVE_RING_ADD(serial_recv.start, ret - FRAME_HEADER_SIZE);
          serial_recv.cnt -= (ret - FRAME_HEADER_SIZE);
          RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                              "get candidate frame header. start_pos: "
                                  << serial_recv.start << " ret: " << ret);
          serial_recv.state = RECV_STATE_CHECK_FIXED_HEADER;
        } else {
          RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                              "Doesnt find header in recv buffer.");
      ...
      case RECV_STATE_CHECK_FIXED_HEADER: {
        ...
        if ((candidate.cmd > CMD_MAX) || (-1 == length) ||
          ...
          /*not a valid header*/
          serial_recv.start =
              RECEIVE_RING_ADD(serial_recv.start, FRAME_HEADER_SIZE);
          ...
          RCLCPP_INFO_STREAM(
              m_node_ptr->get_logger(),
              "get cmd: " << int(candidate.cmd)
                          << " cmd_length: " << candidate.cmd_length);
        }
        serial_recv.state = RECV_STATE_RECEIVING_CRC;
      } break;
      case RECV_STATE_RECEIVING_CRC: {
        RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                            "In state RECV_STATE_RECEIVING_CRC");
        if (serial_recv.cnt < candidate.cmd_length + 2)  // total frame
        {
          RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                              "Need more data to find crc.");
          return;
        }
        candidate_pos =
            RECEIVE_RING_ADD(serial_recv.start, candidate.cmd_length);
        ...
        if (frame_crc == candidate.crc16) {
          RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                             "crc check ok, found a valid frame. "
                                 << "pos: " << serial_recv.start);
          if (!parse_recv_frame()) {
            RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                               "Recv frame queue high level detected, cnt: "
                                   << recv_frame_queue.cnt);
          } else {
            RCLCPP_INFO_STREAM(
                m_node_ptr->get_logger(),
                "Get new frame in recv frame queue***, total cnt: "
                    << int(recv_frame_queue.cnt));
          }
          serial_recv.start =
              RECEIVE_RING_ADD(serial_recv.start, candidate.cmd_length + 2);
          serial_recv.cnt -= (candidate.cmd_length + 2);
          serial_recv.state = RECV_STATE_FINDING_HEADER;
        } else {
          RCLCPP_ERROR_STREAM(m_node_ptr->get_logger(),
                              "bad crc check recv:0x"
                                  << candidate.crc16 << " calc:0x" << frame_crc
      ...
      default: {
        RCLCPP_ERROR_STREAM(
            m_node_ptr->get_logger(),
            "Oops!***, wrong recv state: " << serial_recv.state);
        serial_recv.state = RECV_STATE_FINDING_HEADER;
      } break;
    }
  }
...
void McuOta::process_received_serial_frames() {
  if (recv_frame_queue.cnt >= 3) {
    RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                        "High process_received_serial_frames level detected: "
                            << recv_frame_queue.cnt);
  }
...
int McuOta::get_firmware_packet_data(unsigned char *dest, uint32_t file_offset,
                                     uint16_t length) {
  int ret = 0;
  if (NULL == dest) {
    return -1;
...
int McuOta::update_firmware_upgrade_params(unsigned short require_packet) {
  ...
  fw_packet.read_offset =
      fw_packet.packet_length * (fw_packet.current_packet - 1);
  // test
  RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                      "fw_packet.current_packet: " << fw_packet.current_packet);
  RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                      "total_packets: " << fw_packet.total_packets);
  RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                      "packet_length: " << fw_packet.packet_length);
  RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                      "actual_length: " << fw_packet.actual_length);
  RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                      "read_offset: " << fw_packet.read_offset);
  return 0;
}
//接收指令进程管理
  ...
  switch (frame->cmd) {
    case UPD_CMD_212: {
      ...
      upd_cmd_212_type *upd_cmd_212 =
          (upd_cmd_212_type *)&(frame->data.upd_cmd_212);
      if (upd_cmd_212->update_node == update_option_.main_module_id) {
        RCLCPP_INFO_STREAM(
            m_node_ptr->get_logger(),
            "the main module id respond is right! go to update!");
        firmware_upgrade_flag = FIRMWARE_NORMAL;
      } else {
        ota_state->set_report_information(
            UPDATE_FAILED, firmware_upgrade_flag,
            " the main module id respond is wrong! abort update!");
        firmware_upgrade_flag = FIRMWARE_ABORT_UPGRADE;
      }
    } break;
    case UPD_CMD_210: {
      ...
      upd_cmd_210_type *upd_cmd_210 =
          (upd_cmd_210_type *)&(frame->data.upd_cmd_210);
      RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
      ...
      if (upd_cmd_210->run_area < GET_ARRAY_ELE_NUM(run_area_string)) {
        RCLCPP_INFO_STREAM(
            m_node_ptr->get_logger(),
            "run_area: " << run_area_string[upd_cmd_210->run_area]);
      } else {
        RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                           "run_area: " << upd_cmd_210->run_area);
      }
      if (upd_cmd_210->update_status <
          GET_ARRAY_ELE_NUM(target_status_string)) {
        RCLCPP_INFO_STREAM(
            m_node_ptr->get_logger(),
            "update_status: "
        ...
        RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                           "update_status: " << upd_cmd_210->update_status);
      }
      RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                         "boot_ver: " << upd_cmd_210->boot_ver);
      RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                         "app_ver: " << upd_cmd_210->app_ver);
      RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                         "update_lib_ver: " << upd_cmd_210->update_lib_ver);
      RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                         "*************************GET Firmware Version "
                         "Info*****************************");
      if (firmware_upgrade_flag == FIRMWARE_WAIT_CHECK_BEFORE_VERSION) {
        ota_state->upgrade_report.app_version_before =
            (char *)upd_cmd_210->app_ver;
        firmware_upgrade_flag = FIRMWARE_CHECK_VERSION_OK;
      }
      if (firmware_upgrade_flag == FIRMWARE_WAIT_CHECK_AFTER_VERSION) {
        ota_state->upgrade_report.app_version_after =
            (char *)upd_cmd_210->app_ver;
    ...
    case UPD_CMD_200: /*app, to notify a firmware upgrade available.*/
    {
      ...
      if ((firmware_upgrade_flag != FIRMWARE_PRE_UPDATE) &&
          (firmware_upgrade_flag != FIRMWARE_WAIT_START_UPDATE)) {
        RCLCPP_ERROR_STREAM(
            m_node_ptr->get_logger(),
            "Receive UPD_CMD_200 with wrong state : " << firmware_upgrade_flag);
        ota_state->set_report_information(UPDATE_FAILED, firmware_upgrade_flag,
                                          "Receive cmd_200 in wrong state.");
        // firmware_upgrade_flag = FIRMWARE_ABORT_UPGRADE;
        return -1;
      }
      upd_cmd_200_type *upd_cmd_200 =
          (upd_cmd_200_type *)&(frame->data.upd_cmd_200);
      if (1 == upd_cmd_200->status) {
        RCLCPP_DEBUG_STREAM(
            m_node_ptr->get_logger(),
            "**********Receive acceptment to firmware upgrade. Change state to "
            "FIRMWARE_WAIT_START_UPDATE.**********");
        firmware_upgrade_flag = FIRMWARE_WAIT_START_UPDATE;
      } else {
        RCLCPP_ERROR_STREAM(m_node_ptr->get_logger(),
                            "**********Receive decline to firmware upgrade: "
                                << upd_cmd_200->status
                                << frame->data.upd_cmd_200.version[0]);
        ota_state->set_report_information(UPDATE_FAILED, firmware_upgrade_flag,
                                          "Receive decline to upgrade.");
        firmware_upgrade_flag = FIRMWARE_ABORT_UPGRADE;
      }
    } break;
    case UPD_CMD_202: /*boot, stm32 to pi, to notify it can receive upgrade data
                         now.*/
    {
      clear_cmd_overtime_clock(UPD_CMD_200);
      clear_resend_cmd(UPD_CMD_200);
      ...
      get_firmware_packet_data(upd_cmd_203.data, fw_packet.read_offset,
                               fw_packet.actual_length);
      construct_serial_frame(&serial_frame, UPD_CMD_203, (void *)&upd_cmd_203);
      pack_send_frame_buf_in_queue(&serial_frame,
                                   (UPD_CMD_203 << 8) + MAX_CMD_RESEND_TIME);
    ...
    case UPD_CMD_204: /*boot, stm32 to pi, reply CMD_203 transfer results.*/
    {
      ...
      upd_cmd_204_type *upd_cmd_204 =
          (upd_cmd_204_type *)&(frame->data.upd_cmd_204);
      /*get last packet transfer results*/
      /*last packet transfer ok*/
      if (0 == upd_cmd_204->status) {
        RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                           "Receive UPD_CMD_204 ok status, current_packet: "
        ...
        if (upd_cmd_204->current_packet < fw_packet.total_packets) {
          RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                             "Receive OK result for flashing packet: "
                                 << upd_cmd_204->current_packet
                                 << ", send next packet");
          ...
          upd_cmd_203.packet_length = fw_packet.packet_length;
          upd_cmd_203.actual_length = fw_packet.actual_length;
          /*read databuf from file here: fw_packet.read_offset,
           * fw_packet.actual_length*/
          get_firmware_packet_data(upd_cmd_203.data, fw_packet.read_offset,
                                   fw_packet.actual_length);
          construct_serial_frame(&serial_frame, UPD_CMD_203,
                                 (void *)&upd_cmd_203);
          pack_send_frame_buf_in_queue(
              &serial_frame, (UPD_CMD_203 << 8) + MAX_CMD_RESEND_TIME);
          set_cmd_overtime_clock(UPD_CMD_203, MAX_CMD_OVERTIME_VALUE);
        } else {
          /*when =, this means the last packet is responsed, send reboot
           * command*/
          RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                             "**********Transfer upgrade data complete, send "
                             "reboot command.**********");
          ...
          construct_serial_frame(&serial_frame, UPD_CMD_205,
                                 (void *)&upd_cmd_205);
          pack_send_frame_buf_in_queue(
              &serial_frame, (UPD_CMD_205 << 8) + MAX_CMD_RESEND_TIME);
          set_cmd_overtime_clock(UPD_CMD_205, MAX_CMD_OVERTIME_VALUE * 3);
          firmware_upgrade_flag = FIRMWARE_UPDATED_REBOOT;
          RCLCPP_INFO_STREAM(
              m_node_ptr->get_logger(),
              "In state FIRMWARE_UPDATED_REBOOT send upd_cmd_205.");
        }
      } else if (1 == upd_cmd_204->status) { /*mismatch packet order, but cannot
                                                deal the unresponse transmit*/
        RCLCPP_INFO_STREAM(
            m_node_ptr->get_logger(),
        ...
        /*packet data length not ok*/ /*shold not happen, this is not the
                                         deal.*/
        RCLCPP_ERROR_STREAM(
            m_node_ptr->get_logger(),
        ...
        /*write flash CRC failed*/ /*error, upgrade failed*/
        RCLCPP_ERROR_STREAM(
            m_node_ptr->get_logger(),
        ...
        RCLCPP_ERROR_STREAM(
            m_node_ptr->get_logger(),
            "upd_cmd_204->status: " << upd_cmd_204->status
                                    << " upd_cmd_204->current_packet: "
                                    << upd_cmd_204->current_packet);
        ota_state->set_report_information(UPDATE_FAILED, firmware_upgrade_flag,
                                          "Receive UPD_CMD_204 unkown status.");
    ...
    case UPD_CMD_211: {
      ...
      RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                         "time_recv_cmd_211:" << time_recv_cmd_211);
...
void McuOta::send_serial_frames() {
  ...
  if (send_frame_queue.cnt >= (MAX_SEND_FRAME_QUEUE / 4) * 3) {
    RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                        "High send_frame_queue level detected!!!");
  }
  while (send_frame_queue.cnt > 0) {
    serial_send_buf_type *frame =
        &(send_frame_queue.frame[send_frame_queue.head]);
    ...
    if (send_frame_queue.state == NORMAL_SEND) {
      ...
      if (resend > 0 &&
          (m_node_ptr->get_clock()->now().seconds() - time_timeout) > 1.5) {
        time_timeout = m_node_ptr->get_clock()->now().seconds();
        send_serial_flow(frame->buf, frame->cnt);
        send_frame_queue.resend[send_frame_queue.head]--;
        RCLCPP_DEBUG_STREAM(m_node_ptr->get_logger(),
                            "resend out a frame buf.");
      } else if (resend <= 0) {
        send_frame_queue.state = END_SEND;
        RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                           "The proc get response, to complete.");
      } else {
        return;
      }
    } else if (send_frame_queue.state == END_SEND) {
      RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                         "set no resend. to complete.");
      send_frame_queue.resend[send_frame_queue.head] = 0;
      send_frame_queue.head =
          (send_frame_queue.head + 1) % MAX_SEND_FRAME_QUEUE;
      send_frame_queue.cnt--;
      send_frame_queue.state = NORMAL_SEND;
    } else {
      RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                         "send_frame_queue state error!");
      send_frame_queue.resend[send_frame_queue.head] = 0;
      send_frame_queue.state = NORMAL_SEND;
    }
  }
...
void McuOta::send_data() {
  ...
  if (send_buf_size > 0) {
    ...
    RCLCPP_INFO_STREAM(
        m_node_ptr->get_logger(),
        "send msg, seq_num: " << msg.seq_num << " size: " << msg.data.size());
  }
}
...
int McuOta::send_serial_flow(unsigned char *src, int cnt) {
  ...
  RCLCPP_INFO_STREAM(
      m_node_ptr->get_logger(),
      "ros_buffer_write size: " << cnt << ",ros_send start: " << ros_send.start
                                << ",end: " << ros_send.end);
  return cnt;
}
//当前时间
...
int McuOta::run() {
  ...
  bool size_parame =
      (4 == input_parameters_.size()) || (5 == input_parameters_.size());
  if (!size_parame) {
    RCLCPP_ERROR_STREAM(m_node_ptr->get_logger(), "Input param error.");
    Info("Input param error.");
    RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                       "--Usage: rosrun mcu_ota mcu_ota PORT_NAME "
                       "MAIN_MODULE_NAME MODULE_NAME BIN_FILE_NAME");
    RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                       "PORT_NAME: force to DEFAULT_PORT");
    RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                       "MAIN_MODULE_NAME: main module name to upgrade");
    RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                       "MODULE_NAME: module name to upgrade");
    RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                       "MAIN_MODULE_NAME and MODULE_NAME belongs to the list:");
    for (int i = 0; i < MODULE_PARAM_TABLE_ELE_NUM; i++) {
      RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                         module_param_table[i].module_name);
    }
    RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                       "BIN_FILE_NAME: the bin file for update.");
  ...
  //--
  update_option_.main_module_id =
      check_user_update_option(update_option_.main_module_name);
  if (update_option_.main_module_id < 0) {
    RCLCPP_ERROR_STREAM(m_node_ptr->get_logger(),
                        "User input main module name invalid, exit mcu_ota!");
    Info("User input main module name invalid, exit mcu_ota!");
    return UPDATE_FAILED;
  }
  update_option_.module_id =
      check_user_update_option(update_option_.module_name);
  if (update_option_.module_id < 0) {
    RCLCPP_ERROR_STREAM(m_node_ptr->get_logger(),
                        "User input module name invalid, exit mcu_ota!");
    Info("User input module name invalid, exit mcu_ota!");
    return UPDATE_FAILED;
  }
  if (access(update_option_.bin_file_name.c_str(), F_OK) != 0) {
    RCLCPP_ERROR_STREAM(m_node_ptr->get_logger(),
                        "User input bin file no exists, exit mcu_ota!");
  ...
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "main_module_name: " << update_option_.main_module_name);
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "main_module_id: " << update_option_.main_module_id);
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "module_name: " << update_option_.module_name);
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "module_id: " << update_option_.module_id);
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "bin_file_name: " << update_option_.bin_file_name);
  ...
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "Waiting for ros node sync, start in 3 seconds");
  for (int i = 0; i < 3; i++) {
    RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                       "Start in " << (3 - i) << " seconds");
    sleep(1);
  }
  ...
  while (rclcpp::ok()) {
    manage_module_update_ret = manage_module_updates(
        update_option_.module_id, update_option_.main_module_id);
  ...
  RCLCPP_INFO_STREAM(m_node_ptr->get_logger(),
                     "manage_module_update_ret: " << manage_module_update_ret);
