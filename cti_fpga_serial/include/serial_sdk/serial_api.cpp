...
#define UDP_LOW_RECEIVE_BUFFER_THRESHOLD 1000   
...
void find_serial_frame(int fd)
{
    ...
    //printf("Finding a frame from total %d buffer chars, new received %d chars, start pos:%d.\n", serial_recv.cnt, ret, serial_recv.start);
    //recv_pthread_cnt++;
    ...
    while(serial_recv.cnt)
    {
        switch(serial_recv.state)
        {
          case RECV_STATE_FINDING_HEADER:
              ////////printf("In state RECV_STATE_FINDING_HEADER*********************\n");
              if(serial_recv.cnt <= FRAME_HEADER_SIZE)
              {
                  //printf("Need more data to find header.\n");
                  return;
              }
              ...
              if(found_header) //find header bytes
              {
                  ...
                  //printf("get candidate frame header. start_pos: %d, ret: %d\n", serial_recv.start, ret);
          ...
          case RECV_STATE_CHECK_FIXED_HEADER:
              //printf("In state RECV_STATE_CHECK_FIXED_HEADER*********************\n");
              if(serial_recv.cnt < sizeof(serial_fixed_header_type))
              {
                  ////printf("Need more data to find fixed header.\n");
                  return;
              }
...
#endif
              {
                  //printf("Received cmd[%d] and cmd_length[%d] mismatched! Expect cmd_id<%d, length=%d(%d+%d). But accept when in code developing.\n", \
          ...
          case RECV_STATE_RECEIVING_CRC:
              //printf("In state RECV_STATE_RECEIVING_CRC*********************\n");
              if(serial_recv.cnt < candidate.cmd_length + 2) //total frame
              {
                  //printf("Need more data to find crc.\n");
                  return;
              }
              ...
              if(frame_crc == candidate.crc16)
              {
                  ...
                  recv_crc_status = 0;
                  /*copy data to s_Msg*/
                  //printf("crc check ok, found a valid frame.***, pos:%d.\n", serial_recv.start);
                  if(!parse_recv_frame())
                  {
                      /*the frame queue is full*/
                      //printf("Recv frame queue high level detected, cnt:%d.\n", recv_frame_queue.cnt);
                  }
                  else
                  {
                      //printf("Get new frame in recv frame queue***, total cnt:%d.\n", recv_frame_queue.cnt);
                  }
              ...
              else
              {
				  ...
				  recv_crc_status = -1;
                  printf("bad crc check(recv:0x%x, calc:0x%x). Find a fake or damaged frame.\n", candidate.crc16, frame_crc);
                  serial_recv.start = RECEIVE_RING_ADD(serial_recv.start, FRAME_HEADER_SIZE);
                  serial_recv.cnt -= FRAME_HEADER_SIZE;
                  serial_recv.state = RECV_STATE_FINDING_HEADER;
              }
...
void pack_send_frame_buf(serial_frame_type *frame)
{
    ...
    //test
    //printf("the following frame buf is packed:\n");
...
void process_received_serial_frames(void)
{
   ...
   // std::cout<<"process_received_serial_frames finish"<<std::endl;
}
...
{
    //std::cout<<"send_serial_frames_if_necessary"<<std::endl;
   if(snd_frame_queue.cnt >=4)
    ...
    //std::cout<<"send_serial_frames_if_necessary finish"<<std::endl;
}
...
{
//    std::cout<<"process cmd: "<<(int)frame->cmd<<std::endl;
    if(NULL == frame)
    {
        return -1;
    }
...
// the code below is for udp commiucate
int query_udp_data(int fd)
{
    int pos = 0;
    int remain_buffer_cnt = 0;
    int ret;
...
#if 0 
    /*low available buffer level*/
    if(remain_buffer_cnt < UDP_LOW_RECEIVE_BUFFER_THRESHOLD)
    {
        DEBUG_PRINTF("Low recv buffer level!****cleaning recv buffer!\n\r");
        init_recv_buffer();      
        ret = 0;
    ...
    {
        /*remain_buffer_cnt > (RECEIVE_BUFFER_SIZE - pos)*/
        ret = recv_udp_flow(fd, serial_recv.buf + pos,  ((RECEIVE_BUFFER_SIZE - pos) > SERIAL_FRAME_SIZE) ? \
                                SERIAL_FRAME_SIZE : (RECEIVE_BUFFER_SIZE - pos));
    }
    else
    {
        ret = recv_udp_flow(fd, serial_recv.buf + pos,  (remain_buffer_cnt > SERIAL_FRAME_SIZE) ? \
                                SERIAL_FRAME_SIZE : remain_buffer_cnt); 
    }
#endif
    //在udp通讯中，每次读取之后，不管读取多少，缓存里的数据都会被释放，使用环形队列按上面的逻辑进行接收的话。会造成数据接收不全的情况
    //因此改成下面的接收方式，每次都是全部接受。
    init_recv_buffer();  
    ret = recv_udp_flow(fd, serial_recv.buf + pos, SERIAL_FRAME_SIZE);
...
int send_single_udp_frame(serial_frame_type *frame, unsigned char need_setting)
{
    int fd, ret;
    ...
    printf("\n");
    */
    ret = send_udp_flow(fd, serial_snd.buf, serial_snd.cnt);
    DEBUG_PRINTF("Send serial frame, total %d bytes.\n", ret);
...
int send_update_udp_frame(const unsigned char *data, unsigned int len)
{
    int fd = -1, ret = -1;
    fd = module_serial_fd;
    if(fd >= 0){
        ret = send_udp_flow(fd, (unsigned char*)data, len);
    }
    return ret;
}
...
void send_udp_frames_if_necessary(void)
{
   if(snd_frame_queue.cnt >=4)
    {
        DEBUG_PRINTF("High snd_frame_queue level detected!!!\n");
    ...
    {    
        ...
        {
            case NORMAL_SEND: //send a frame;
                send_udp_flow(module_serial_fd, frame->buf, frame->cnt); 
                DEBUG_PRINTF("send out a frame buf.\n");
                if(snd_frame_queue.resend[snd_frame_queue.head] != 0)
                {
                    gettimeofday(&tv, NULL);
...
void module_udp_init(void)
{
    init_frame_queue();
    init_recv_buffer();
    module_serial_fd = open_udp_port();
    //udp_dest_fd = open_udp_dest_port();
}
...
void* udp_recv_thread(void* arg) 
{
    while(1)
    {
        find_udp_frame(module_serial_fd);
        usleep(2000);
    }
    return NULL;
}
...
/*process_received_serial_frames() and find_serial_frame(fd) must in same thread,
  or the send_frame_queue.cnt will conflict.*/
void module_udp_process_thread(void)
{
    process_received_serial_frames();
    send_udp_frames_if_necessary();
}
