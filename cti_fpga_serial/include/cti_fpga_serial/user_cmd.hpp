...
}process_twork_mode_t;

//*************************3_0_超声波类型话题名称枚举
std::string ult_3_0_name[ULT_3_0_TYPE_NUM] = {
...
std::string ult_name[max_type_ult] = {
    "left_front_ult", "front_left_ult", "front_center_ult", "front_right_ult",
    "right_front_ult", "back_center_ult", "right_rear_ult", "rear_right_ult",
    //"rear_center_ult",
    "rear_left_ult", "left_rear_ult", "left_center_ult",
...
std::string sweeper_ult_name[sweeper_max_type_ult] = {
    "sweeper_left_up",    "sweeper_center_up",   "sweeper_right_up",
    "sweeper_right_down", "sweeper_center_down", "sweeper_left_down"};
//**************************新版超声波名称枚举
...
//***************************新版超声波话题名称枚举
std::string
    new_sweeper_ult_name[SWEEPER_ULT_MODULE_NUM][new_sweeper_max_type_ult] = {
        "sweeper_left_front",  "sweeper_left_up",      "sweeper_center_down",
        "sweeper_right_front", "sweeper_left_center",  "sweeper_center_up",
        "sweeper_right_down",  "sweeper_right_center", "sweeper_left_rear",
        "sweeper_left_down",   "sweeper_right_up",     "sweeper_right_rear"};
...
typedef struct __attribute__((packed)) {
  ...
  motor_status_t motor_status;
  uint8_t recv_dustbin_on_the_car;
  uint8_t dam_board_status;            //挡板状态: 0:升 1:降           //2:OPEN 1:CLOAE 3:AUTO
...
///////////////////////////////////////////////////////
//************************接受到集尘箱的状态(新的,增加电池状态,风机转矩等)
// struct Dust_box_to_motion_t_new_2()
// {
//     uint8_t infrared_data; //红外数据
//     uint8_t anti_pinch_data;　//防夹数据
//     float stay_cord_distance;　//拉绳距离

// }dust_box_to_motion_t_new_2;

// typedef struct Dust_box_to_motion_t_new_2 {
//   // uint8_t infrared_data; //红外数据
//   // uint8_t anti_pinch_data;//防夹数据
//   // float stay_cord_distance;//拉绳距离
//     uint8_t infrared_data;     // 0[read]  1[write]
...
extern int (*process_nomal_cmd_cb)(unsigned char cmd, unsigned char *data,
                                   unsigned int length);
extern int (*process_update_cmd_cb)(unsigned char cmd, unsigned char *data,
                                    unsigned int length);

extern int construct_serial_frame_ex(serial_frame_type *frame,
                                     unsigned char cmd, unsigned int length,
                                     void *data);
extern int pack_send_frame_buf_in_queue(serial_frame_type *frame,
                                        unsigned short resend);
extern int send_single_serial_frame(serial_frame_type *frame,
                                    unsigned char need_setting);
