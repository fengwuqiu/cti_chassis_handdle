...
//  嵌入式各个版本信息命令号
void ChassisHanddle::init_check_arr() {
  std::array<int, 2> check_arr{};

  check_arr = {5, 20};
  check_arr_deque.push_back(check_arr);
  check_arr = {5, 40};
  check_arr_deque.push_back(check_arr);
  check_arr = {5, 48};
  check_arr_deque.push_back(check_arr);
...
void ChassisHanddle::initSub() {
  ...
  light_v3_0_sub = this->create_subscription<cti_msgs::msg::DataArray>(
      "cti/fpga_serial/light_type_v3_0", rclcpp::QoS{20},
  ...
  checkversion_sub = this->create_subscription<std_msgs::msg::UInt32MultiArray>(
  ...
  errorcode_clean_sub =
      this->create_subscription<std_msgs::msg::UInt32MultiArray>(
          "/cti/chassis_handdle/errorcode_clean", rclcpp::QoS{1},
          std::bind(&ChassisHanddle::errorcode_clean_callback, this,
                    std::placeholders::_1));

  battery_parameter_setting_sub =
      this->create_subscription<std_msgs::msg::UInt32MultiArray>(
          "/cti/chassis_handdle/battery_parameter_setting", rclcpp::QoS{1},
          std::bind(&ChassisHanddle::battery_parameter_setting_callback, this,
                    std::placeholders::_1));

  common_error_clear_sub =
      this->create_subscription<std_msgs::msg::UInt64MultiArray>(
          "/cti/chassis_handdle/common_error_clear", rclcpp::QoS{1},
          std::bind(&ChassisHanddle::common_error_clear_callback, this,
                    std::placeholders::_1));  // 错误码清空

  extend_transfer_rtk_sub = this->create_subscription<std_msgs::msg::String>(
      "/cti/rtk/differential/data", rclcpp::QoS{1},
      std::bind(&ChassisHanddle::extend_transfer_rtk_Callback, this,
                std::placeholders::_1));  // 监听gnss rtk  ;

  msg_sw_enable_control_sub =
      this->create_subscription<cti_msgs::msg::VehicleCtlRunInfo>(
          "/cti/chassis_serial/bar_state", rclcpp::QoS{1},
          std::bind(&ChassisHanddle::msg_sw_enable_control_Callback, this,
                    std::placeholders::_1));
  msg_platform_configurations_sub =
      this->create_subscription<std_msgs::msg::String>(
          "/platform_communication/misc/platform/configurations",
          rclcpp::QoS{1},
          std::bind(&ChassisHanddle::msg_platform_configurations_Callback, this,
                    std::placeholders::_1));
}
...
void ChassisHanddle::initPub() {
  ...
  rtk_pub = this->create_publisher<cti_msgs::msg::GnssRTK>(
      "/cti/chassis_serial/rtk", rclcpp::QoS{1});

  shallow_sleep_pub = this->create_publisher<std_msgs::msg::Bool>(
      "/cti/chassis/shallow_sleep", rclcpp::QoS{1});

  error_status_pub = this->create_publisher<cti_msgs::msg::ErrorStatus>(
      "/cti/error_status/set", rclcpp::QoS{1});

  board_version_pub = this->create_publisher<cti_msgs::msg::BoardVersionData>(
      "/cti/cti_chassis/board_version", rclcpp::QoS{1});

  gga_data_pub = this->create_publisher<std_msgs::msg::String>(
      "/cti/chassis_serial/gga", rclcpp::QoS{1});
}
...
void ChassisHanddle::initTimer() {
  ...
  timer_board_version_check = this->create_wall_timer(30s, [this]() {
    this->timer_board_version_check_Callback();
  });  // 版本查询（开机30s后开始查）

  timer_board_version_send = this->create_wall_timer(40s, [this]() {
    this->timer_board_version_send_Callback();
  });  // 版本发布 (开机40s后开始发布）
}
...
void ChassisHanddle::initLog(const std::string name) {
  ...
  Logger::getLogger().setMaxSize(500 * 1024 * 1024);
}
...
void ChassisHanddle::poweroff_cmd_type_callback(
  ...
  if (msg->data.size() == 4) {
    send_to_poweroff_cmd_type_new_.restart_control = msg->data[3];
  }
...
void ChassisHanddle::errorcode_clean_callback(
    const std_msgs::msg::UInt32MultiArray::SharedPtr msg) {
  clear_errorinfo clear_errorinfo_{0};
  clear_errorinfo_.cmd = msg->data[0];
  clear_errorinfo_.cleardata = msg->data[1];

  std::cout << "errorcode_clean_callback  clear_errorinfo_.cmd:"
            << clear_errorinfo_.cmd
            << "  clear_errorinfo_.cleardata:" << clear_errorinfo_.cleardata
            << std::endl;

  chassisCmncBase->queueSendDatas(CLEAR_ERRORINFO, sizeof(clear_errorinfo),
                                  &clear_errorinfo_, false);
}

void ChassisHanddle::battery_parameter_setting_callback(
    const std_msgs::msg::UInt32MultiArray::SharedPtr msg) {
  msg_battery_parameter_set_3_0 msg_battery_parameter_set_3_0_{0};
  msg_battery_parameter_set_3_0_.cmd = msg->data[0];
  msg_battery_parameter_set_3_0_.data_len = msg->data[1];

  std::cout << "battery_parameter_setting_callback  "
               "msg_battery_parameter_set_3_0_.cmd:"
            << msg_battery_parameter_set_3_0_.cmd
            << "  msg_battery_parameter_set_3_0_.data_len:"
            << msg_battery_parameter_set_3_0_.data_len << std::endl;

  chassisCmncBase->queueSendDatas(BATTERRY_PARAMETER_SET,
                                  sizeof(msg_battery_parameter_set_3_0),
                                  &msg_battery_parameter_set_3_0_, false);
}

//************************************** 清空错误码回调函数
void ChassisHanddle::common_error_clear_callback(
    const std_msgs::msg::UInt64MultiArray::SharedPtr msg) {
  msg_common_error_clear_3_0 msg_common_error_clear_3_0{0};

  msg_common_error_clear_3_0.module_id = msg->data[0];
  msg_common_error_clear_3_0.cmd = msg->data[1];
  msg_common_error_clear_3_0.value = msg->data[2];
  std::cout << "battery_parameter_setting_callback  "
               "msg_battery_parameter_set_3_0_.cmd:"
            << msg_common_error_clear_3_0.cmd
            << "  msg_battery_parameter_set_3_0_.data_len:"
            << msg_common_error_clear_3_0.value << std::endl;
  chassisCmncBase->queueSendDatas(RECV_ERROR_CODE_CLEAR,
                                  sizeof(msg_common_error_clear_3_0),
                                  &msg_common_error_clear_3_0, false);
}

//************************************** 触边蜂鸣器状态触发 回调函数
void ChassisHanddle::msg_sw_enable_control_Callback(
    const cti_msgs::msg::VehicleCtlRunInfo::SharedPtr msg) {
  msg_sw_enable_control_1_0 msg_sw_enable_control_1_0_;
  msg_sw_enable_control_1_0_.control_mode.data = msg->box_sw;
  std::cout << "msg_sw_enable_control::" << msg->box_sw << std::endl;

  chassisCmncBase->queueSendDatas(MSG_SW_ENABLE_CONTROL,
                                  sizeof(msg_sw_enable_control_1_0),
                                  &msg_sw_enable_control_1_0_, false);
}

//************************************** 云端配置接口 回调函数
void ChassisHanddle::msg_platform_configurations_Callback(
    const std_msgs::msg::String::SharedPtr msg) {
  Json::Value root;
  Json::Reader reader;
  msg_sw_enable_control_1_0 msg_sw_enable_control_1_0_{0};

  if (reader.parse(msg->data, root)) {
    // 满溢超声开关
    if (root.isMember("overflowUltrasoundSwitch") &&
        root["overflowUltrasoundSwitch"].isBool() &&
        (!root["overflowUltrasoundSwitch"].asBool())) {
      overflowUltrasoundSwitch_state = false;
    } else {
      overflowUltrasoundSwitch_state = true;
    }

    // 左前超声开关
    if (root.isMember("leftFrontUltrasoundSwitch") &&
        root["leftFrontUltrasoundSwitch"].isBool() &&
        (!root["leftFrontUltrasoundSwitch"].asBool())) {
      leftFrontUltrasoundSwitch_state = false;
    } else {
      leftFrontUltrasoundSwitch_state = true;
    }

    // 右前超声开关
    if (root.isMember("rightFrontUltrasoundSwitch") &&
        root["rightFrontUltrasoundSwitch"].isBool() &&
        (!root["rightFrontUltrasoundSwitch"].asBool())) {
      rightFrontUltrasoundSwitch_state = false;
    } else {
      rightFrontUltrasoundSwitch_state = true;
    }

    // 右中前超声开关
    if (root.isMember("rightMiddleFrontUltrasoundSwitch") &&
        root["rightMiddleFrontUltrasoundSwitch"].isBool() &&
        (!root["rightMiddleFrontUltrasoundSwitch"].asBool())) {
      rightMiddleFrontUltrasoundSwitch_state = false;
    } else {
      rightMiddleFrontUltrasoundSwitch_state = true;
    }

    // 右中后超声开关
    if (root.isMember("rightMiddleBackUltrasoundSwitch") &&
        root["rightMiddleBackUltrasoundSwitch"].isBool() &&
        (!root["rightMiddleBackUltrasoundSwitch"].asBool())) {
      rightMiddleBackUltrasoundSwitch_state = false;
    } else {
      rightMiddleBackUltrasoundSwitch_state = true;
    }

    // 右后超声开关
    if (root.isMember("rightBackUltrasoundSwitch") &&
        root["rightBackUltrasoundSwitch"].isBool() &&
        (!root["rightBackUltrasoundSwitch"].asBool())) {
      rightBackUltrasoundSwitch_state = false;
    } else {
      rightBackUltrasoundSwitch_state = true;
    }

    // 左后超声开关
    if (root.isMember("leftBackUltrasoundSwitch") &&
        root["leftBackUltrasoundSwitch"].isBool() &&
        (!root["leftBackUltrasoundSwitch"].asBool())) {
      leftBackUltrasoundSwitch_state = false;
    } else {
      leftBackUltrasoundSwitch_state = true;
    }

    // 左中后超声开关
    if (root.isMember("leftMiddleBackUltrasoundSwitch") &&
        root["leftMiddleBackUltrasoundSwitch"].isBool() &&
        (!root["leftMiddleBackUltrasoundSwitch"].asBool())) {
      leftMiddleBackUltrasoundSwitch_state = false;
    } else {
      leftMiddleBackUltrasoundSwitch_state = true;
    }

    // 左中前超声开关
    if (root.isMember("leftMiddleFrontUltrasoundSwitch")) {
      if (root["leftMiddleFrontUltrasoundSwitch"].isBool() &&
          (!root["leftMiddleFrontUltrasoundSwitch"].asBool())) {
        leftMiddleFrontUltrasoundSwitch_state = false;
      } else {
        leftMiddleFrontUltrasoundSwitch_state = true;
      }
    }

    // 前触边开关
    if (root.isMember("beforeTouchEdgeSwitch") &&
        root["beforeTouchEdgeSwitch"].isBool() &&
        (!root["beforeTouchEdgeSwitch"].asBool())) {
      msg_sw_enable_control_1_0_.control_mode.bits.front_bar = 1;

      chassisCmncBase->queueSendDatas(MSG_SW_ENABLE_CONTROL,
                                      sizeof(msg_sw_enable_control_1_0),
                                      &msg_sw_enable_control_1_0_, false);
    } else {
      msg_sw_enable_control_1_0_.control_mode.bits.front_bar = 0;
      chassisCmncBase->queueSendDatas(MSG_SW_ENABLE_CONTROL,
                                      sizeof(msg_sw_enable_control_1_0),
                                      &msg_sw_enable_control_1_0_, false);
    }
    ...
    // 后触边开关
    if (root.isMember("afterTouchEdgeSwitch") &&
        root["afterTouchEdgeSwitch"].isBool() &&
        (!root["afterTouchEdgeSwitch"].asBool())) {
      // msg_sw_enable_control_1_0_.control_mode.data |= 0x02;
      msg_sw_enable_control_1_0_.control_mode.bits.rear_bar = 1;
      chassisCmncBase->queueSendDatas(MSG_SW_ENABLE_CONTROL,
                                      sizeof(msg_sw_enable_control_1_0),
                                      &msg_sw_enable_control_1_0_, false);

    } else {
      msg_sw_enable_control_1_0_.control_mode.bits.rear_bar = 0;
      chassisCmncBase->queueSendDatas(MSG_SW_ENABLE_CONTROL,
                                      sizeof(msg_sw_enable_control_1_0),
                                      &msg_sw_enable_control_1_0_, false);
    }
  ...
  Info("满溢超声开关: "
       << overflowUltrasoundSwitch_state
       << " 左前超声开关: " << leftFrontUltrasoundSwitch_state
       << " 右前超声开关: " << rightFrontUltrasoundSwitch_state
       << " 右中前超声开关: " << rightMiddleFrontUltrasoundSwitch_state
       << " 右中后超声开关: " << rightMiddleBackUltrasoundSwitch_state
       << " 右后超声开关: " << rightBackUltrasoundSwitch_state
       << " 左后超声开关: " << leftBackUltrasoundSwitch_state
       << " 左中后超声开关: " << leftMiddleBackUltrasoundSwitch_state
       << " 左中前超声开关: " << leftMiddleFrontUltrasoundSwitch_state
       << " 前后触边值:  " << msg_sw_enable_control_1_0_.control_mode.data);
}

//************************************** 扩展传输rtk 回调函数
void ChassisHanddle::extend_transfer_rtk_Callback(
    const std_msgs::msg::String::SharedPtr msg) {
  std::cout << "进入rkt差分" << std::endl;
  std::string gnzz_data(msg->data);
  // gnzz_data = msg->data;
  // gnzz_data = "fhiuasdiuh";
  // for (int i = 0; i < 6; i++) {
  //   gnzz_data = gnzz_data + gnzz_data;
  // }
  std::cout << gnzz_data << std::endl;
  int vaule_all_size = gnzz_data.length();
  init_crc();
  uint16_t vaule_crc =
      calc_crc((unsigned char *)(gnzz_data.c_str()), vaule_all_size);
  int pack_size = 243;  // 243  1包长度

  int quotient = (vaule_all_size) / (pack_size);   // 商
  int remainder = (vaule_all_size) % (pack_size);  // 余数
  if (remainder == 0) {                            // 包总数
    quotient = quotient;
  } else {
    quotient = quotient + 1;
  ...
  for (int i = 0; i < quotient; i++) {
    if (i == (quotient - 1)) {
      std::string value_once =
          gnzz_data.substr(vaule_all_size - remainder, vaule_all_size);
      msg_extend_transfer_3_0 msg_extend_transfer_3_0_{0};
      msg_extend_transfer_3_0_.route_info.sour = 2;
      msg_extend_transfer_3_0_.route_info.dest = 2;
      msg_extend_transfer_3_0_.extend_cmd = 1;
      msg_extend_transfer_3_0_.pack_amount = quotient;
      msg_extend_transfer_3_0_.pack_data_len = vaule_all_size;
      msg_extend_transfer_3_0_.pack_crc = vaule_crc;
      msg_extend_transfer_3_0_.transfer_id;
      msg_extend_transfer_3_0_.pack_num = (quotient - 1);
      msg_extend_transfer_3_0_.payload_len = remainder;
      for (int j = 0; j < remainder; j++) {
        msg_extend_transfer_3_0_.payload[j] = value_once[j];
      }
      printf("最后一包xxx~%d", msg_extend_transfer_3_0_.pack_amount);
      printf("包号~%d", msg_extend_transfer_3_0_.pack_num);
      std::cout << "总长度" << msg_extend_transfer_3_0_.pack_data_len
                << "有效数据长度" << msg_extend_transfer_3_0_.payload_len
                << "包内容" << msg_extend_transfer_3_0_.payload << std::endl;
      chassisCmncBase->queueSendDatas(RECV_MSG_EXTND_TRANSFER_3_0,
                                      sizeof(msg_extend_transfer_3_0),
                                      &msg_extend_transfer_3_0_, false);
    } else {
      std::string value_once = gnzz_data.substr(pack_size * i, pack_size);
      msg_extend_transfer_3_0 msg_extend_transfer_3_0_{0};
      msg_extend_transfer_3_0_.route_info.sour = 2;
      msg_extend_transfer_3_0_.route_info.dest = 2;
      msg_extend_transfer_3_0_.extend_cmd = 1;
      ...
      msg_extend_transfer_3_0_.pack_crc = vaule_crc;
      msg_extend_transfer_3_0_.transfer_id;
      msg_extend_transfer_3_0_.pack_num = i;
      msg_extend_transfer_3_0_.payload_len = pack_size;
      ...
      chassisCmncBase->queueSendDatas(RECV_MSG_EXTND_TRANSFER_3_0,
                                      sizeof(msg_extend_transfer_3_0),
                                      &msg_extend_transfer_3_0_, false);
      printf("总包xxx数~%d", msg_extend_transfer_3_0_.pack_amount);
      printf("包号~%d", msg_extend_transfer_3_0_.pack_num);
      std::cout << "总长度" << msg_extend_transfer_3_0_.pack_data_len
                << "有效数据长度" << msg_extend_transfer_3_0_.payload_len
                << "包内容" << msg_extend_transfer_3_0_.payload << std::endl;
...
void ChassisHanddle::dustBoxControlInfo_Callback(
  ...
  for (int i = 0; i < msg->datas.size(); i++) {
    ...
    else if (info_msg.name == "port")
      // port uint8_t
      send_to_dust_box_cmd.port =
          atoi(info_msg.data.c_str()) + 1;  // 接收到数值+1
    else
      continue;
  }
...
void ChassisHanddle::checkversion_Callback(
  ...
  ////////////////////////////////////////////////先发送212
  msg_ota_212_1_0 msg_ota_212_1_0_{0};
  update_info_type update_info_type_{0};
  update_info_type_.src = MODULE_CONTROL_PC;
  update_info_type_.dest = MODULE_MOVE_CONTROL_BOARD;
  msg_ota_212_1_0_.route_info = update_info_type_;
  msg_ota_212_1_0_.node = msg->data[0];

  chassisCmncBase->queueSendDatas(UPD_CMD_212, sizeof(msg_ota_212_1_0),
                                  &msg_ota_212_1_0_, false);
  ...
  update_info_struct.dest = msg->data[1];
...
void ChassisHanddle::cleanControlInfo_Callback(
  ...
  for (int i = 0; i < msg->datas.size(); i++) {
    ...
    else if (info_msg.name == "free_sidebrush_Twining")
      // led灯 uint8_t 1   0
      send_to_clean_cmd_new.free_sidebrush_Twining =
  ...
  Info("S_CC_ES: "
       ...
       << " S_CC_LED: " << (int)send_to_clean_cmd_new.free_sidebrush_Twining
       << " S_CC_DOC: " << (int)send_to_clean_cmd_new.dustbin_on_car
...
void ChassisHanddle::sprayControl_Callback(
  ...
  send_to_clean_cmd_new.dust_suppresion = 0;
  send_to_clean_cmd_new.side_brush = 0;
  send_to_clean_cmd_new.free_sidebrush_Twining = 0;
...
void ChassisHanddle::wirelessCharge_Callback(
  ...
  Info("S_WC: " << (int)msg->data[0]);
  Info("S_CC: " << (int)msg->data[1]);
  send_battery_board.soft_stop = 0;  // 　软急停　０：无作为　１：复位　
  send_battery_board.wireless_charge = msg->data[0];
  send_battery_board.charge_current = msg->data[1];
...
void ChassisHanddle::pub_alt_3_0_(const msg_upa_pos_data_t *data) {
  operation_ultrasonic_time.time =
      rclcpp::Clock().now().seconds();  // 超声波时间
  cti_chassis_msgs::msg::UltV30Datas ult_datas;

  // 右中前超声波
  if (rightMiddleFrontUltrasoundSwitch_state) {
    ult_datas.ori_datas.data.push_back((double)data->upa_data[2]);
  } else {
    ult_datas.ori_datas.data.push_back(9000.0);
  }

  // 右中后超声波
  if (rightMiddleBackUltrasoundSwitch_state) {
  ...
  // 左中后超声波
  if (leftMiddleBackUltrasoundSwitch_state) {
    ult_datas.ori_datas.data.push_back((double)data->upa_data[6]);
  } else {
    ult_datas.ori_datas.data.push_back(9000.0);
  }
  // 左中前超声波
  if (leftMiddleFrontUltrasoundSwitch_state) {
    ult_datas.ori_datas.data.push_back((double)data->upa_data[7]);
  } else {
    ult_datas.ori_datas.data.push_back(9000.0);
  }
  // 左前超声波
  if (leftFrontUltrasoundSwitch_state) {
    ult_datas.ori_datas.data.push_back((double)data->upa_data[0]);
  } else {
    ult_datas.ori_datas.data.push_back(9000.0);
  }
  // 右前超声波
  if (rightFrontUltrasoundSwitch_state) {
    ult_datas.ori_datas.data.push_back((double)data->upa_data[1]);
  } else {
    ult_datas.ori_datas.data.push_back(9000.0);
  }
  // 右后超声波
  if (rightBackUltrasoundSwitch_state) {
    ult_datas.ori_datas.data.push_back((double)data->upa_data[4]);
  } else {
    ult_datas.ori_datas.data.push_back(9000.0);
  }
  // 左后超声波
  if (leftBackUltrasoundSwitch_state) {
    ult_datas.ori_datas.data.push_back((double)data->upa_data[5]);
  } else {
    ult_datas.ori_datas.data.push_back(9000.0);
  }
...
void ChassisHanddle::pub_odom(const recv_from_control_status_type *data) {
  ...
  chassis_chat_state.time_recv =
      rclcpp::Clock().now().seconds();  // 收到运控信息的时间
  //---------------------------odom_4wd publish
  tf2::Quaternion q_tf;
  ...
  operation_control_time.time =
      rclcpp::Clock().now().seconds();  // 远程运维-收到运控信息的时间
  ...
  Info("R_OC_SV: " << (int)data->state_vehicle
                   << " R_OC_SVE: " << (int)data->drivers_enable
                   << " R_OC_CME: " << (int)data->control_mode
                   << " R_OC_SBE: " << (int)data->state_brake
                   << " R_OC_VLX: " << (float)data->vel_liner_x
                   << " R_OC_VLY: " << (float)data->vel_liner_y
                   << " R_OC_VAR: " << (float)data->vel_angular
                   << " R_BT_AFT: " << (float)data->angle_front_turn
                   << " R_OC_ART: " << (float)data->angle_rear_turn
                   << " R_OC_SSD: " << (int)data->sw_status.data
                   << " R_OC_OR: "  << (double)data->odometer
                   << " R_OC_PX: "  << (double)data->pos.x
                   << " R_OC_PY: "  << (double)data->pos.y
                   << " R_OC_AYR: " << (float)data->angle_yaw_radian
                   << " R_OC_LPN: " << (int)data->lift_position
                   << " R_OC_BRW: " << (float)data->baro_raw
                   << " R_OC_BHT: " << (float)data->baro_height
                   << " R_OC_SFG: " << (int)data->state_flag);
}
...
void ChassisHanddle::pub_battery_3_0(
  ...
  operation_battery_time.time =
      rclcpp::Clock().now().seconds();  // 接收到电池消息的时间
  ...
  BatCellsState.charge_current = data->battery_charger_current;  // 充电电流
  ...
  BatCellsState.signal_pin = data->signal_pin;  // 信号针
  ...
  Info(" R_WC_VT: " << (int)data->Bat_1_Volt
                    ...
                    << " R_BT_CG_SPN: " << (int)data->signal_pin
                    << " R_BT_CG_CRE: " << (int)data->charge_reverse
                    << " R_BT_CG_VLE: " << (int)data->v_leakage
                    << " R_BT_CG_BSS: " << (int)data->brake_status
                    << " R_BT_CG_CRSE: " << (int)data->charge_reverse
                    << " R_BT_CG_FCY: " << (float)data->full_capacity
                    << " R_BT_CG_SCY: " << (float)data->surplus_capacity
...
void ChassisHanddle::pub_firmwareversion(
  ...
  board_check_version board_check_version;  // 嵌入式板子版本信息结构体
  board_check_version.board_id = data->upd_info.src;
  board_check_version.boot_ver = data->boot_ver;
  ;
  board_check_version.app_ver = data->app_ver;
  ;
  board_check_version.update_lib_ver = data->update_lib_ver;
  board_check_version_vec.push_back(board_check_version);
...
void ChassisHanddle::pub_chassiserror(recv_chassis_error_report_type *data) {
  ...
  Info("R_CER_MT: " << (int)data->module_type
                    << " R_CER_MEC: " << (int)data->module_error_code
                    << " R_CER_MEL: " << (int)data->module_error_level);
}
//*************************************Timer5定时器，发布地盘错误码
...
void ChassisHanddle::pub_navigationlog(recv_navigation_log_status_type *data) {
  ...
  std::string value_set_torque("[");
  std::string value_now_encoder("[");
  for (int i = 0; i < (sizeof(data->set_torque) / sizeof(int16_t)); i++) {
    navigation_log.set_torque.push_back(data->set_torque[i]);

    if (i == (sizeof(data->set_torque) / sizeof(int16_t) - 1)) {
      value_set_torque += std::to_string(data->set_torque[i]) + "]";
    } else {
      value_set_torque += std::to_string(data->set_torque[i]) + ",";
    }
  }
  for (int i = 0; i < (sizeof(data->now_encoder) / sizeof(uint16_t)); i++) {
    navigation_log.now_encoder.push_back(data->now_encoder[i]);

    if (i == (sizeof(data->now_encoder) / sizeof(uint16_t) - 1)) {
      value_now_encoder += std::to_string(data->now_encoder[i]) + "]";
    } else {
      value_now_encoder += std::to_string(data->now_encoder[i]) + ",";
    }
  }
  ...
  Info("R_CH_PIX: "
       << (int)data->portIndex << " R_CH_LSD: " << (float)data->liner_speed
       << " R_CH_TAE: " << (float)data->turn_angle << " R_CH_BTE: "
       << (int)data->break_torque << " R_CH_EFG: " << (int)data->enable_flag
       << " R_CH_ABLX: " << (float)data->actual_body_linear_vel_x
       << " R_CH_ASBLW: " << (float)data->actual_speed_base_on_left_front_wheel
       << " R_CH_ASBRW: " << (float)data->actual_speed_base_on_right_front_wheel
       << " R_CH_ASBLRW: " << (float)data->actual_speed_base_on_left_rear_wheel
       << " R_CH_ASBRRW: " << (float)data->actual_speed_base_on_right_rear_wheel
       << " R_CH_ATFAE: " << (float)data->actual_turn_front_angle
       << " R_CH_ATRAE: " << (float)data->actual_turn_rear_angle
       << " R_CH_STD: " << (std::string)value_set_torque
       << " R_CH_NER: " << (std::string)value_now_encoder);
...
void ChassisHanddle::pub_gps(recv_gps_data_type *data) {
  ...
  operation_gnss_time.time = rclcpp::Clock().now().seconds();  // gnss时间
...
void ChassisHanddle::pub_msg_vehicle_kinematic_log_3_0(
    msg_vehicle_kinematic_log_3_0 *data) {
  if (!data) {
    return;
  }
  std::string value_set_torque("[");
  ...
  Info("R_MVK_CLST" << (float)data->chassis_linear_set
                    << " R_MVK_CTAS: " << (float)data->chassis_turn_angle_set
                    << " R_MVK_CHC: " << (float)data->chassis_acc
                    << " R_MVK_BTE: " << (float)data->brake_torque
                    << " R_MVK_CTTE: " << (float)data->chassis_total_torque
                    << " R_MVK_CFVL: " << (float)data->chassis_fusion_vel
                    << " R_MVK_FLWVL: " << (float)data->front_left_wheel_vel
                    << " R_MVK_FLWAC: " << (float)data->front_left_wheel_acc
                    << " R_MVK_FLWSP: " << (float)data->front_left_wheel_slip
                    << " R_MVK_FLWBSD: "
                    << (float)data->front_left_wheel_base_speed
                    << " R_MVK_FLWTVL: " << (float)data->front_left_wheel_torque
                    << " R_MVK_FRWVL: " << (float)data->front_right_wheel_vel
                    << " R_MVK_FRWAC: " << (float)data->front_right_wheel_acc
                    << " R_MVK_FRWSP: " << (float)data->front_right_wheel_slip
                    << " R_MVK_FRWTE: " << (float)data->front_right_wheel_torque
                    << " R_MVK_STE: " << (std::string)value_set_torque
                    << " R_MVK_NER: " << (std::string)value_now_encoder);
}

void ChassisHanddle::pub_msg_extend_transfer_3_0(
    msg_extend_transfer_3_0 *data) {
  // static pack_num = data->pack_num;
  // static pack_len = data->pack_data_len;
  // static pack_crc = data->pack_crc;

  // if ((pack_num == data->pack_num) && (pack_len == data->pack_data_len)) {
  //   if ((data->pack_amount) <= pack_num) {
  //     std::map<int, msg_extend_transfer_3_0> map_pack;
  //     map_pack[(int)data->pack_amount] = data;
  //   }

  //   if (map_pack.size() == pack_num) {
  //     std::string gga_data;
  //     for (std::map<int, msg_extend_transfer_3_0>::iterator iter =
  //              map_pack.begin();
  //          iter != map_pack.end(); ++iter) {
  //       gga + = iter->first.payload;
  //     }
  //     init_crc();
  //      uint16_t vaule_crc = calc_crc((unsigned char *)(gnzz.c_str()),
  //      vaule_all_size);
  //     if ((gga.size() == string_num) && (pack_crc==vaule_crc) ) {
  //       publish();
  //     }else{
  //       map_pack.clear();
  //     }
  ...
  //   pack_len =data->pack_data_len;
  //   pub_msg_extend_transfer_3_0(data);
  // }
  if (!data) {
    return;
  }
  std::string gga_data((char *)data->payload);

  std_msgs::msg::String gga_data_;
  gga_data_.data = gga_data;
  std::cout << "gga::" << gga_data << std::endl;
  gga_data_pub->publish(gga_data_);  // 发布gga报文信息
}
...
void ChassisHanddle::pub_msg_dustbox_msg_report_cmd(
    msg_dustbox_msg_report_cmd_1_0 *data) {
  if (!data) {
    return;
  }

  std::string msg_data("[");
  for (int i = 0; i < (sizeof(data->msg1) / sizeof(uint8_t)); i++) {
    if (i == (sizeof(data->msg1) / sizeof(uint8_t) - 1)) {
      msg_data += std::to_string(data->msg1[i]) + "]";
    } else {
      msg_data += std::to_string(data->msg1[i]) + ",";
    }
  }

  Info(" D_DUST_PIX: " << (int)data->portIndex
                       << " D_DUST_MSGDATA: " << (std::string)msg_data);
}

void ChassisHanddle::pub_msg_dustbin_temperature_info(
    msg_dustbin_temperature_info_3_0 *data) {
  if (!data) {
    return;
  }

  Info(" D_DUST_TEMP1: " << (float)data->temp_1
                         << " D_DUST_TEMP2: " << (float)data->temp_2
                         << " D_DUST_TEMP3: " << (float)data->temp_3
                         << " D_DUST_TEMP4: " << (float)data->temp_4
                         << " D_DUST_TEMP5: " << (float)data->temp_5
                         << " D_DUST_TEMP6: " << (float)data->temp_6
                         << " D_DUST_TEMP7: " << (float)data->temp_7
                         << " D_DUST_TEMP8: " << (float)data->temp_8
                         << " D_DUST_TEMP9: " << (float)data->temp_9
                         << " D_DUST_TEMP10: " << (float)data->temp_10
                         << " D_DUST_TEMP11: " << (float)data->temp_11
                         << " D_DUST_TEMP12: " << (float)data->temp_12
                         << " D_DUST_TEMP13: " << (float)data->temp_13
                         << " D_DUST_TEMP14: " << (float)data->temp_14
                         << " D_DUST_TEMP15: " << (float)data->temp_15
                         << " D_DUST_TEMP16: " << (float)data->temp_16
                         << " D_DUST_TEMP17: " << (float)data->temp_17
                         << " D_DUST_TEMP18: " << (float)data->temp_18
                         << " D_DUST_TEMP19: " << (float)data->temp_19
                         << " D_DUST_TEMP20: " << (float)data->temp_20);
}

void ChassisHanddle::pub_msg_dustbin_current_info(
    msg_dustbin_current_info_3_0 *data) {
  if (!data) {
    return;
  }

  Info(" D_DUST_CURRENT1: " << (float)data->current_1
                            << " D_DUST_CURRENT2: " << (float)data->current_2
                            << " D_DUST_CURRENT3: " << (float)data->current_3
                            << " D_DUST_CURRENT4: " << (float)data->current_4
                            << " D_DUST_CURRENT5: " << (float)data->current_5
                            << " D_DUST_CURRENT6: " << (float)data->current_6
                            << " D_DUST_CURRENT7: " << (float)data->current_7
                            << " D_DUST_CURRENT8: " << (float)data->current_8
                            << " D_DUST_CURRENT9: " << (float)data->current_9
                            << " D_DUST_CURRENT10: " << (float)data->current_10
                            << " D_DUST_CURRENT11: " << (float)data->current_11
                            << " D_DUST_CURRENT12: " << (float)data->current_12
                            << " D_DUST_CURRENT13: " << (float)data->current_13
                            << " D_DUST_CURRENT14: " << (float)data->current_14
                            << " D_DUST_CURRENT15: " << (float)data->current_15
                            << " D_DUST_CURRENT16: " << (float)data->current_16
                            << " D_DUST_CURRENT17: " << (float)data->current_17
                            << " D_DUST_CURRENT18: " << (float)data->current_18
                            << " D_DUST_CURRENT19: " << (float)data->current_19
                            << " D_DUST_CURRENT20: "
                            << (float)data->current_20);
}
...
void ChassisHanddle::pub_clean_state_new(clean_to_motion_t_new *data) {
  ...
  dustVehicleInfos.datas.push_back(dustVehicleInfo);
  // 防火检测
  dustVehicleInfo.name = "fire_prevent";
  uint8_t fire_prevent = data->multi_status.bits.fire_prevent;
  dustVehicleInfo.data = std::to_string(fire_prevent);
  dustVehicleInfo.type = cti_msgs::msg::Data::TYPE_UINT8;
  dustVehicleInfos.datas.push_back(dustVehicleInfo);
  // 边刷防缠绕1
  dustVehicleInfo.name = "anti_winding1";
  uint8_t anti_winding1 = data->multi_status.bits.anti_winding1;
  dustVehicleInfo.data = std::to_string(anti_winding1);
  dustVehicleInfo.type = cti_msgs::msg::Data::TYPE_UINT8;
  dustVehicleInfos.datas.push_back(dustVehicleInfo);
  // 边刷防缠绕2
  dustVehicleInfo.name = "anti_winding2";
  uint8_t anti_winding2 = data->multi_status.bits.anti_winding2;
  dustVehicleInfo.data = std::to_string(anti_winding2);
  dustVehicleInfo.type = cti_msgs::msg::Data::TYPE_UINT8;
  dustVehicleInfos.datas.push_back(dustVehicleInfo);
  // 震尘速度
  ...
  operation_clean_time.time =
      rclcpp::Clock().now().seconds();  // 远程运维-清扫通信时间
  Info("R_CC_ES: " << (int)data->motor_status.data
                   ...
                   << " R_CC_STUD: " << (int)data->multi_status.data
                   << " R_CC_WP: " << (int)vehicle_water_status.water_percnt_now
                   << " R_CC_GSE: " << (int)data->roll_brush_error_code
                   << " R_CC_CMS: " << (int)data->control_mode_state
                   << " R_CC_GSS: " << (int)data->roll_brush_speed
...
void ChassisHanddle::pub_dust_box_state_new(dust_box_to_motion_t_new *data) {
  ...
  if (!overflowUltrasoundSwitch_state) {
    dustboxinfo.data = "0";
  }
  dustBoxInfos.datas.push_back(dustboxinfo);
  // 电池1的电压
  ...
  Info("R_DBO_ES: " << (int)data->motor_status.data
                    << " R_DBO_IDA: " << (int)data->infrared_data
                    << " R_DBO_APDA: " << (int)data->anti_pinch_data
                    << " R_DBO_PIX: " << (int)data->portIndex
                    << " R_DBO_AW: " << (int)data->auto_work
                    << " R_DBO_MW: " << (int)data->mannual_work
                    << " R_DBO_DS: " << (int)data->dustbin_distance
                    << " R_DBO_UD0: " << (int)data->ultrasonic_distance[0]
                    << " R_DBO_UD1: " << (int)data->ultrasonic_distance[1]
                    << " R_DBO_UD2: " << (int)data->ultrasonic_distance[2]
                    << " R_DBO_UD3: " << (int)data->ultrasonic_distance[3]
                    << " R_DBO_UD4: " << (int)data->ultrasonic_distance[4]
                    << " R_DBO_UD5: " << (int)data->ultrasonic_distance[5]
                    << " R_DBO_DOC: " << (int)data->recv_dustbin_on_the_car
                    << " R_DBO_AP: " << (int)data->auto_push
                    << " R_DBO_FS: " << (int)data->fan_speed
                    << " R_DBO_WC_VT: " << (float)data->wireless_voltage
                    << " R_DBO_WC_CU: " << (float)data->wireless_current
                    ...
                    << " R_DBO_BAT1: " << (float)data->voltage1
                    << " R_DBO_SOC1: " << (int)data->Bat_1_Soc
                    << " R_DBO_BAT2: " << (float)data->voltage2
                    << " R_DBO_SOC2: " << (int)data->Bat_2_Soc
                    << " R_DBO_BAT3: " << (float)data->voltage3
                    << " R_DBO_SOC3: " << (int)data->Bat_3_Soc
                    << " R_DBO_BAT4: " << (float)data->voltage4
                    ...
                    << " R_DBO_PW_SCD: " << (float)data->stay_cord_distance
...
void ChassisHanddle::pub_rain_sensor_new(rain_sensor_t_new *data) {
  operation_small_screen_time.time =
      rclcpp::Clock().now().seconds();  // 收到小屏幕信息的时间
...
//
void ChassisHanddle::pub_bat_report_type_3_0(msg_bat_report_type_3_0 *data) {
  // std::cout<<"========================================"<<std::endl;
  // std::cout<<"battery_id "<<(double)data->battery_id<<std::endl;
  // std::cout<<"cell_number "<<(double)data->cell_number<<std::endl;
  // for(int i{0}; i < 16 ; i++)
  // {
  //   std::cout<<"cell_state"<<i<<" :
  //   "<<(double)data->cell_state[i]<<std::endl;
  // }
  // for(int i{0}; i < 16 ; i++)
  // {
  //   std::cout<<"cell_voltage"<<i<<" :
  //   "<<(double)data->cell_voltage[i]<<std::endl;
  // }
  // std::cout<<"ntc_number "<<(double)data->ntc_number<<std::endl;
  // for(int i{0}; i < 7 ; i++)
  // {
  //   std::cout<<"cell_teampture_state"<<i<<" :
  //   "<<(double)data->cell_teampture_state[i]<<std::endl;
  // }
  // for(int i{0}; i < 7 ; i++)
  // {
  //   std::cout<<"cell_teampture"<<i<<" :
  //   "<<(double)data->cell_teampture[i]<<std::endl;
  // }
  // std::cout<<"bat_current "<<(double)data->bat_current<<std::endl;
  // std::cout<<"bat_voltage "<<(double)data->bat_voltage<<std::endl;
  // std::cout<<"full_capacity "<<(double)data->full_capacity<<std::endl;
  // std::cout<<"cycle_index "<<(double)data->cycle_index<<std::endl;
  // std::cout<<"soc "<<(double)data->soc<<std::endl;
  // std::cout<<"soh "<<(double)data->soh<<std::endl;
  // std::cout<<"equalization "<<(double)data->equalization<<std::endl;

  // for(int i{0}; i < 8 ; i++)
  // {
  //   std::cout<<"user_state"<<i<<" :
  //   "<<(double)data->user_state[i]<<std::endl;
  // }y
}

void ChassisHanddle::pub_msg_battery_parameter_report_3_0(
    msg_battery_parameter_report_3_0 *data) {
  // std::cout<<"========================================"<<std::endl;
  // std::cout<<"seq "<<(double)data->seq<<std::endl;
  // std::cout<<"status "<<(double)data->status<<std::endl;
  // std::cout<<"data_len "<<(double)data->data_len<<std::endl;
  // for(int i{0}; i < 32 ; i++)
  // {
  //   std::cout<<"data"<<i<<" : "<<(double)data->data[i]<<std::endl;
  // }
}
...
bool ChassisHanddle::if_timeout(double time_now, double time_monitor,
                                double timeout_time) {
  double result_time = time_now - time_monitor;
  if (result_time >= timeout_time) {
    return true;
  } else {
    return false;
  }
...
void ChassisHanddle::timer_chat_timeout_Callbak() {
  ...
  // 远程运维监控
  cti_msgs::msg::ErrorStatus errorstatus;
  operation_control_time.time_now = operation_clean_time.time_now =
      operation_battery_time.time_now = operation_gnss_time.time_now =
          operation_ultrasonic_time.time_now =
              operation_small_screen_time.time_now =
                  rclcpp::Clock().now().seconds();
  double time_timeout = 2;  // 判断超时时间

  // 运控通信异常
  if (if_timeout(operation_control_time.time_now, operation_control_time.time,
                 time_timeout)) {
    errorstatus.module_name = "COMM_ERR";
    errorstatus.error_code = 1;
    errorstatus.error_info = "运控通信异常";
    error_status_pub->publish(errorstatus);
    usleep(3000);

    // IMU数据通信异常
    errorstatus.module_name = "COMM_ERR";
    errorstatus.error_code = 9;
    errorstatus.error_info = "IMU数据通信异常";
    error_status_pub->publish(errorstatus);
    usleep(3000);

    // 里程计数据通信异常
    errorstatus.module_name = "COMM_ERR";
    errorstatus.error_code = 10;
    errorstatus.error_info = "里程计数据通信异常";
    error_status_pub->publish(errorstatus);
    usleep(3000);

    operation_control_time.time = operation_control_time.time_now;
  }

  // 清扫模块通信异常
  if (if_timeout(operation_clean_time.time_now, operation_clean_time.time,
                 time_timeout)) {
    errorstatus.module_name = "COMM_ERR";
    errorstatus.error_code = 2;
    errorstatus.error_info = "清扫模块通信异常";
    error_status_pub->publish(errorstatus);
    usleep(3000);

    // 倾倒机构通信异常
    errorstatus.module_name = "COMM_ERR";
    errorstatus.error_code = 3;
    errorstatus.error_info = "倾倒机构通信异常";
    error_status_pub->publish(errorstatus);
    usleep(3000);

    operation_clean_time.time = operation_clean_time.time_now;
  }

  // 电池信息通信异常
  if (if_timeout(operation_battery_time.time_now, operation_battery_time.time,
                 time_timeout * 2)) {
    errorstatus.module_name = "COMM_ERR";
    errorstatus.error_code = 7;
    errorstatus.error_info = "电池信息通信异常";
    error_status_pub->publish(errorstatus);
    usleep(3000);

    operation_battery_time.time = operation_battery_time.time_now;
  }

  // GNSS数据通信异常
  if (if_timeout(operation_gnss_time.time_now, operation_gnss_time.time,
                 time_timeout)) {
    errorstatus.module_name = "COMM_ERR";
    errorstatus.error_code = 8;
    errorstatus.error_info = "GNSS数据通信异常";
    error_status_pub->publish(errorstatus);
    usleep(3000);

    operation_gnss_time.time = operation_gnss_time.time_now;
  }

  // 超声波数据通信异常
  if (if_timeout(operation_ultrasonic_time.time_now,
                 operation_ultrasonic_time.time, time_timeout)) {
    errorstatus.module_name = "COMM_ERR";
    errorstatus.error_code = 14;
    errorstatus.error_info = "超声波数据通信异常";
    error_status_pub->publish(errorstatus);
    usleep(3000);

    operation_ultrasonic_time.time = operation_ultrasonic_time.time_now;
  }

  // 小屏幕通信异常
  if (if_timeout(operation_small_screen_time.time_now,
                 operation_small_screen_time.time, time_timeout)) {
    errorstatus.module_name = "COMM_ERR";
    errorstatus.error_code = 15;
    errorstatus.error_info = "小屏幕通信异常";
    error_status_pub->publish(errorstatus);
    usleep(3000);

    operation_small_screen_time.time = operation_small_screen_time.time_now;
  }

  // 运控接口未识别
  if (!chassisCmncBase->get_connect_state()) {
    errorstatus.module_name = "SENSOR_ERR";
    errorstatus.error_code = 4;
    errorstatus.error_info = "运控接口未识别";
    error_status_pub->publish(errorstatus);
    usleep(3000);
  }
}
...
// 查询嵌入式版本与发布
void ChassisHanddle::timer_board_version_check_Callback() {
  //************************************** 查询固件版本号命令话题
  if (stm32_update_flag) {
    return;
  }

  if (0 != check_arr_deque.size()) {
    state_check = false;
    ////////////////////////////////////////////////先发送212
    msg_ota_212_1_0 msg_ota_212_1_0_{0};
    update_info_type update_info_type_{0};
    ...
    update_info_type_.dest = MODULE_MOVE_CONTROL_BOARD;
    msg_ota_212_1_0_.route_info = update_info_type_;
    msg_ota_212_1_0_.node = check_arr_deque[0][0];
    ...
    update_info_type update_info_struct;
    update_info_struct.src = MODULE_CONTROL_PC;
    update_info_struct.dest = check_arr_deque[0][1];
    check_version_cmd.upd_info = update_info_struct;
    check_version_cmd.check = 01;
    ...
    check_arr_deque.pop_front();
    timer_board_version_check = this->create_wall_timer(
        std::chrono::milliseconds(500),
        [this]() { this->timer_board_version_check_Callback(); });
  } else {
    state_check = true;
    board_check_version_vec_send = board_check_version_vec;
    init_check_arr();
    board_check_version_vec.clear();
    timer_board_version_check = this->create_wall_timer(
        600s, [this]() { this->timer_board_version_check_Callback(); });
  }
}

// 嵌入式版本发布
void ChassisHanddle::timer_board_version_send_Callback() {
  if (state_check) {
    cti_msgs::msg::BoardVersionData board_info_msg;
    for (auto i : board_check_version_vec_send) {
      cti_msgs::msg::BoardVersion board_info;
      board_info.board_id = i.board_id;
      board_info.boot_ver = i.boot_ver;
      board_info.app_ver = i.app_ver;
      board_info.update_lib_ver = i.update_lib_ver;
      board_info_msg.boardversions.push_back(board_info);
    }
    board_version_pub->publish(board_info_msg);
    timer_board_version_send = this->create_wall_timer(1s, [this]() {
      this->timer_board_version_send_Callback();
    });  //  每秒发布版本
  }
}
...
int ChassisHanddle::process_nomal_cmd(unsigned char cmd, unsigned int length,
  ...
  switch (cmd) {
    case DUST_BOX_5G_RECV_CMD:
      msg_dustbox_msg_report_cmd_1_0 *msg_dustbox_msg_report_cmd_1_0_;
      if (sizeof(msg_dustbox_msg_report_cmd_1_0) != length) {
        std::cout << "recv msg_dustbox_msg_report_cmd_1_0 length: " << length
                  << " error! sizeof struct length: "
                  << sizeof(msg_dustbox_msg_report_cmd_1_0) << std::endl;
        return -1;
      }
      msg_dustbox_msg_report_cmd_1_0_ = (msg_dustbox_msg_report_cmd_1_0 *)data;
      pub_msg_dustbox_msg_report_cmd(msg_dustbox_msg_report_cmd_1_0_);
      break;
    case DUST_BOX_5G_TEMP_INFO:
      msg_dustbin_temperature_info_3_0 *msg_dustbin_temperature_info_3_0_;
      if (sizeof(msg_dustbin_temperature_info_3_0) != length) {
        std::cout << "recv msg_dustbin_temperature_info_3_0 length: " << length
                  << " error! sizeof struct length: "
                  << sizeof(msg_dustbin_temperature_info_3_0) << std::endl;
        return -1;
      }
      msg_dustbin_temperature_info_3_0_ =
          (msg_dustbin_temperature_info_3_0 *)data;
      pub_msg_dustbin_temperature_info(msg_dustbin_temperature_info_3_0_);
      break;
    case DUST_BOX_5G_CURRENT:
      msg_dustbin_current_info_3_0 *msg_dustbin_current_info_3_0_;
      if (sizeof(msg_dustbin_current_info_3_0) != length) {
        std::cout << "recv msg_dustbin_current_info_3_0 length: " << length
                  << " error! sizeof struct length: "
                  << sizeof(msg_dustbin_current_info_3_0) << std::endl;
        return -1;
      }
      msg_dustbin_current_info_3_0_ = (msg_dustbin_current_info_3_0 *)data;
      pub_msg_dustbin_current_info(msg_dustbin_current_info_3_0_);
      break;

    case RECV_MSG_EXTND_TRANSFER_3_0:
      msg_extend_transfer_3_0 *msg_extend_transfer_3_0_;
      if (sizeof(msg_extend_transfer_3_0_) != length) {
        std::cout << "recv msg_extend_transfer_3_0 length: " << length
                  << " error! sizeof struct length: "
                  << sizeof(msg_extend_transfer_3_0_) << std::endl;
        return -1;
      }
      msg_extend_transfer_3_0_ = (msg_extend_transfer_3_0 *)data;
      pub_msg_extend_transfer_3_0(msg_extend_transfer_3_0_);
      break;

    case RECV_VEHILE_KINEMATIC_LOG_3_0:
      msg_vehicle_kinematic_log_3_0 *msg_vehicle_kinematic_log_3_0_;
      if (sizeof(msg_vehicle_kinematic_log_3_0) != length) {
        std::cout << "recv msg_vehicle_kinematic_log_3_0 length: " << length
                  << " error! sizeof struct length: "
                  << sizeof(msg_vehicle_kinematic_log_3_0) << std::endl;
        return -1;
      }
      msg_vehicle_kinematic_log_3_0_ = (msg_vehicle_kinematic_log_3_0 *)data;
      pub_msg_vehicle_kinematic_log_3_0(msg_vehicle_kinematic_log_3_0_);
      break;
    ...
    case RAIN_SENSOR:
      ...
      break;
    case BAT_REPORT_TYPE:
      msg_bat_report_type_3_0 *msg_bat_report_type_3_0_;
      if (sizeof(msg_bat_report_type_3_0) != length) {
        std::cout << "recv msg_bat_report_type_3_0 length: " << length
                  << "size of struct length new: "
                  << sizeof(msg_bat_report_type_3_0) << std::endl;
        return -1;
      }
      if (sizeof(msg_bat_report_type_3_0) == length) {
        msg_bat_report_type_3_0_ = (msg_bat_report_type_3_0 *)data;
        pub_bat_report_type_3_0(msg_bat_report_type_3_0_);
      }
      break;
    case BATTERRY_PARAMETER_REPORT:
      msg_battery_parameter_report_3_0 *msg_battery_parameter_report_3_0_;
      if (sizeof(msg_battery_parameter_report_3_0) != length) {
        std::cout << "recv msg_battery_parameter_report_3_0 length: " << length
                  << "size of struct length new: "
                  << sizeof(msg_battery_parameter_report_3_0) << std::endl;
        return -1;
      }
      if (sizeof(msg_battery_parameter_report_3_0) == length) {
        msg_battery_parameter_report_3_0_ =
            (msg_battery_parameter_report_3_0 *)data;
        pub_msg_battery_parameter_report_3_0(msg_battery_parameter_report_3_0_);
      }
      break;

    case UPD_CMD_212:
      msg_ota_212_1_0 *msg_ota_212_1_0_;
      if (sizeof(msg_ota_212_1_0) != length) {
        std::cout << "recv msg_ota_212_1_0 length: " << length
                  << "size of struct length new: " << sizeof(msg_ota_212_1_0)
                  << std::endl;
        return -1;
      }
      if (sizeof(msg_ota_212_1_0) == length) {
        msg_ota_212_1_0_ = (msg_ota_212_1_0 *)data;
        // pub_msg_battery_parameter_report_3_0(msg_ota_212_1_0_);
        std::cout << "reivece 212 cmd : data.node:"
                  << (int)msg_ota_212_1_0_->node
                  << " data.sourse:" << (int)msg_ota_212_1_0_->route_info.src
                  << " data.dest:" << (int)msg_ota_212_1_0_->route_info.dest
                  << std::endl;
      }
      break;
    default:
