...
//************************************** 清空错误码回调函数
void ChassisHanddle::common_error_clear_callback(const std_msgs::msg::UInt64MultiArray::SharedPtr msg) { 
...
void ChassisHanddle::dustBoxControlInfo_Callback(
  ...
  for (int i = 0; i < msg->datas.size(); i++) {
    ...
    if (info_msg.name == "engin_start")
      ...
      // port uint8_t
      send_to_dust_box_cmd.port = atoi(info_msg.data.c_str())+1;    //接收到数值+1
    else
      continue;
  }
