...
#include <cti_msgs/msg/narrow_road.hpp>
#include <cti_msgs/msg/board_version_data.hpp>
#include <cti_msgs/msg/board_version.hpp>
...
class ChassisHanddle : public rclcpp::Node {
 ...
 private:
  ...
  void wirelessCharge_Callback(const std_msgs::msg::UInt8MultiArray::SharedPtr msg);
  ...
  void errorcode_clean_callback(
      const std_msgs::msg::UInt32MultiArray::SharedPtr msg);

  void battery_parameter_setting_callback(
      const std_msgs::msg::UInt32MultiArray::SharedPtr msg);

  void common_error_clear_callback(
      const std_msgs::msg::UInt64MultiArray::SharedPtr msg);  // 清空错误码

  void extend_transfer_rtk_Callback(
      const std_msgs::msg::String::SharedPtr msg);  // 扩展传输 rtk

  void msg_sw_enable_control_Callback(
      const cti_msgs::msg::VehicleCtlRunInfo::SharedPtr
          msg);  // 触边蜂鸣器状态触发 回调函数

  void msg_platform_configurations_Callback(
     const std_msgs::msg::String::SharedPtr msg);  // 云端配置接口回调函数        
  ...
  void timer_board_version_check_Callback();

  void timer_board_version_send_Callback();
  ...
  void pub_msg_battery_parameter_report_3_0(
      msg_battery_parameter_report_3_0 *data);

  void pub_msg_vehicle_kinematic_log_3_0(
      msg_vehicle_kinematic_log_3_0 *data);  // gnss,rtk 回调函数

  void pub_msg_extend_transfer_3_0(msg_extend_transfer_3_0 *data);  // 扩展传输

  bool if_timeout(double time_now, double time_monitor,
                  double timeout_time);  // 判断超时

  bool getFromJson(const Json::Value &json_value, const std::string name, const bool default_value);  //解析json格式

  void pub_msg_dustbox_msg_report_cmd(msg_dustbox_msg_report_cmd_1_0 *data);

  void pub_msg_dustbin_temperature_info(msg_dustbin_temperature_info_3_0 *data);

  void pub_msg_dustbin_current_info(msg_dustbin_current_info_3_0 *data);
  ...
  rclcpp::Subscription<std_msgs::msg::UInt64MultiArray>::SharedPtr
      common_error_clear_sub;  // 错误码清空

  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr
      extend_transfer_rtk_sub;  // 监听gnss rtk

  rclcpp::Subscription<cti_msgs::msg::VehicleCtlRunInfo>::SharedPtr
      msg_sw_enable_control_sub;  // 触边蜂鸣器状态触发

  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr
      msg_platform_configurations_sub;  //  云端配置接口订阅 
  ////////////////////////////////////////////////////////////////////////////
  ...
  rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr
      shallow_sleep_pub;  // 浅休眠

  rclcpp::Publisher<cti_msgs::msg::ErrorStatus>::SharedPtr
      error_status_pub;  // 错误告警

  rclcpp::Publisher<cti_msgs::msg::BoardVersionData>::SharedPtr
      board_version_pub;  // 发布嵌入式板子版本

  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr
      gga_data_pub;  // gga信息传递
  ...
  rclcpp::TimerBase::SharedPtr timer_board_version_check;
  rclcpp::TimerBase::SharedPtr timer_board_version_send;
  ...
  operation_time operation_control_time;       // 运维-运控时间结构体
  operation_time operation_clean_time;         // 清扫时间
  operation_time operation_battery_time;       // 电池时间
  operation_time operation_gnss_time;          // gnss时间
  operation_time operation_ultrasonic_time;    // 超声波时间
  operation_time operation_small_screen_time;  // 小屏幕时间

  std::deque<std::array<int, 2>> check_arr_deque;  // 嵌入式板子命令集合
  std::vector<board_check_version>
      board_check_version_vec;  // 嵌入式板子版本信息集合
  std::vector<board_check_version>
      board_check_version_vec_send;  // 发送嵌入式板子版本信息集合
  bool state_check{false};   //查询嵌入式版本状态

  bool overflowUltrasoundSwitch_state{true};   // 满溢超声开关
  bool leftFrontUltrasoundSwitch_state{true};  //  左前超声开关
  bool rightFrontUltrasoundSwitch_state{true};  //  右前超声开关
  bool rightMiddleFrontUltrasoundSwitch_state{true};  //  右中前超声开关
  bool rightMiddleBackUltrasoundSwitch_state{true};  //  右中后超声开关
  bool rightBackUltrasoundSwitch_state{true};  //  右后超声开关
  bool leftBackUltrasoundSwitch_state{true};  //  左后超声开关
  bool leftMiddleBackUltrasoundSwitch_state{true};  //  左中后超声开关
  bool leftMiddleFrontUltrasoundSwitch_state{true};  //  左中前超声开关
