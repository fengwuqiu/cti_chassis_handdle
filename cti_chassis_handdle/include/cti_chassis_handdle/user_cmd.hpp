...
enum {
  ...
  MSG_SW_ENABLE_CONTROL = 35,  // 控制触边，蜂鸣器

  DUSBIN_RF_SET_READ_CMD = 40,  // 设定清扫箱箱号,接收清扫箱箱号

  BAT_REPORT_TYPE = 46,
  BATTERRY_PARAMETER_SET = 47,
  BATTERRY_PARAMETER_REPORT = 48,
  CLEAR_ERRORINFO = 49,

  DUST_BOX_5G_CHECK_CMD = 80,  // 查询吸尘箱5g状态
  DUST_BOX_5G_RECV_CMD = 81,   // 接收吸尘箱5g状态   
  DUST_BOX_5G_TEMP_INFO=83,     // 接收吸尘箱温度
  DUST_BOX_5G_CURRENT=85,     //  接收吸尘箱电流
  ...
  RECV_VEHILE_KINEMATIC_LOG_3_0 = 112,  //
  RECV_ERROR_CODE_CLEAR = 119,          // 清空错误码
  RECV_MSG_EXTND_TRANSFER_3_0 = 120,    // 扩展传输
...
typedef struct __attribute__((packed)) {
  ...
  float cmd_vel_Vy;       // y轴线速度send_to_poweroff_cmd_type_new
  float cmd_vel_W;        // 角度
...
typedef struct __attribute__((packed)) {
  ...
  uint32_t restart_control;   // 关机1  重启2
...
typedef struct __attribute__((packed)) {
  /// msg.msg_route_info.0.1 route_info
  update_info_type route_info;
  /// saturated uint8 node
  uint8_t node;
} msg_ota_212_1_0;
...
typedef struct __attribute__((packed)) _battery_board_cmd_s {
  ...
  uint8_t charge_current{0};   //  充电电流大小 0-50   0是让嵌入式端自主控制，大于0是导航设置
  uint8_t unused4{0};
  uint8_t unused5{0};
} battery_board_cmd_t;
...
typedef struct __attribute__((packed)) {
  ...
  uint8_t free_sidebrush_Twining{0};  // 接触边刷参饶报警
...
typedef union {
  ...
  struct {
    ...
    uint8_t fire_prevent : 1;         // 防火检测
    uint8_t anti_winding1 : 1;        //  防缠绕1  左边刷
    uint8_t anti_winding2 : 1;        // 防缠绕2  右边刷
    uint8_t unused4 : 1;
    uint8_t unused5 : 1;
    uint8_t unused6 : 1;
  } bits;
...
typedef struct __attribute__((packed)) {
  ...
  uint8_t dam_board_status;  // 挡板状态: 0:升 1:降           //2:OPEN 1:CLOAE
                             // 3:AUTO
  ...
  uint8_t push_board_state[9];   // 第5位满溢超声自清洁  0正常 1脏-清洁
...
typedef struct __attribute__((packed)) {
  ...
  uint8_t trash_per;
  uint16_t dustbin_distance;  // 超声波垃圾量数值
  uint16_t ultrasonic_distance[6];
  float voltage1;
  uint8_t Bat_1_Soc;
  float voltage2;
...
typedef struct __attribute__((packed)) {
  ...
  float battery_charger_current;  // 充电机实际输出电流   大于0表示在充电
  uint8_t battery_charger_flag;   // 充电标志
  uint8_t battery_charger_state;  // 充电机状态

  uint32_t uused1;  // 预留
  uint32_t uused2;
  uint32_t signal_pin;   // 信号针  1就位
...
typedef struct __attribute__((packed)) {
  ...
  /// saturated uint8 data_len
  uint8_t data_len;

  /// saturated uint8[32] data
  uint8_t data[32];
} msg_battery_parameter_report_3_0;
// 48
...
typedef struct __attribute__((packed)) {
  ...
  /// saturated uint8 cell_number
  uint8_t cell_number;

  /// saturated uint8[16] cell_state
  uint8_t cell_state[16];

  /// saturated float32[16] cell_voltage
  float cell_voltage[16];

  /// saturated uint8 ntc_number
  uint8_t ntc_number;

  /// saturated uint8[7] cell_teampture_state
  uint8_t cell_teampture_state[7];

  /// saturated float32[7] cell_teampture
  float cell_teampture[7];

  /// saturated float32 bat_current
  float bat_current;

  /// saturated float32 bat_voltage
  float bat_voltage;

  /// saturated float32 surplus_capacity
  float surplus_capacity;

  /// saturated float32 full_capacity
  float full_capacity;

  /// saturated uint16 cycle_index
  ...
  /// saturated uint16 equalization
  uint16_t equalization;

  /// saturated uint8[8] user_state
  uint8_t user_state[8];

  /// saturated uint32 uused1
  uint32_t uused1;

  /// saturated uint32 uused2
  uint32_t uused2;

  /// saturated uint32 uused3
  uint32_t uused3;
} msg_bat_report_type_3_0;
// 46

typedef struct __attribute__((packed)) {
  /// saturated float32 chassis_linear_set
  float chassis_linear_set;

  /// saturated float32 chassis_turn_angle_set
  float chassis_turn_angle_set;

  /// saturated float32 chassis_acc
  float chassis_acc;

  /// saturated float32 brake_torque
  float brake_torque;

  /// saturated float32 chassis_total_torque
  float chassis_total_torque;

  /// saturated float32 chassis_fusion_vel
  float chassis_fusion_vel;

  /// saturated float32 front_left_wheel_vel
  float front_left_wheel_vel;

  /// saturated float32 front_left_wheel_acc
  float front_left_wheel_acc;

  /// saturated float32 front_left_wheel_slip
  float front_left_wheel_slip;

  /// saturated float32 front_left_wheel_base_speed
  float front_left_wheel_base_speed;

  /// saturated float32 front_left_wheel_torque
  float front_left_wheel_torque;

  /// saturated float32 front_right_wheel_vel
  float front_right_wheel_vel;

  /// saturated float32 front_right_wheel_acc
  float front_right_wheel_acc;

  /// saturated float32 front_right_wheel_slip
  float front_right_wheel_slip;

  /// saturated float32 front_right_wheel_torque
  float front_right_wheel_torque;

  /// saturated int16[8] set_torque
  int16_t set_torque[8];

  /// saturated uint16[8] now_encoder
  uint16_t now_encoder[8];
} msg_vehicle_kinematic_log_3_0;

typedef struct __attribute__((packed)) {
  /// saturated uint8 module_id
...
// 查询嵌入式板子版本
 typedef struct board_check_version {
  int board_id;                // 唯一板子id
  std::string boot_ver;        // 版本信息
  std::string app_ver;         // 版本信息
  std::string update_lib_ver;  // 版本信息
} board_check_version;


typedef struct __attribute__((packed)) {
  /// saturated uint8 sour
  uint8_t sour;

  /// saturated uint8 dest
  uint8_t dest;
} msg_msg_route_info_0_1;   //地址


//  扩展传输
typedef struct __attribute__((packed)) {  
  /// msg.msg_route_info.0.1 route_info
  msg_msg_route_info_0_1 route_info;

  /// saturated uint16 extend_cmd
  uint16_t extend_cmd;   //0

  /// saturated uint8 pack_amount
  uint8_t pack_amount;    //包id

  /// saturated uint16 pack_data_len
  uint16_t pack_data_len;    //总长度

  /// saturated uint16 pack_crc
  uint16_t pack_crc;  // crc校验

  /// saturated uint8 transfer_id
  uint8_t transfer_id; // 0

  /// saturated uint8 pack_num
  uint8_t pack_num;     //总包数

  /// saturated uint16 payload_len
  uint16_t payload_len;  //实际包内容长度

  /// saturated uint8[243] payload
  uint8_t payload[243];  // 数据内容
} msg_extend_transfer_3_0;


typedef union {
    uint8_t data;
    struct {
      uint8_t front_bar : 1;  // 前触边 0开启触边  1关闭触边   
      uint8_t rear_bar : 1;  //  后触边
      uint8_t buzzer_state : 1;    //  蜂鸣器
      uint8_t unused4 : 1;
      uint8_t unused5 : 1;
      uint8_t unused6 : 1;
      uint8_t unused7 : 1;
...
typedef struct __attribute__((packed)) {
  ...
  /// saturated uint16 unused
  uint16_t unused;
} msg_sw_enable_control_1_0;   // 触边蜂鸣器状态触发



typedef struct __attribute__((packed))
{
    /// saturated uint8 portIndex
    uint8_t portIndex;

    /// saturated uint8[32] msg1
    uint8_t msg1[32];

    /// saturated uint8[32] unused1
    uint8_t unused1[32];

    /// saturated uint8[32] unused2
    uint8_t unused2[32];
} msg_dustbox_msg_report_cmd_1_0;  //81  接收吸尘箱5g状态  


typedef struct __attribute__((packed))
{
    /// saturated float32 temp_1
    float temp_1;

    /// saturated float32 temp_2
    float temp_2;

    /// saturated float32 temp_3
    float temp_3;

    /// saturated float32 temp_4
    float temp_4;

    /// saturated float32 temp_5
    float temp_5;

    /// saturated float32 temp_6
    float temp_6;

    /// saturated float32 temp_7
    float temp_7;

    /// saturated float32 temp_8
    float temp_8;

    /// saturated float32 temp_9
    float temp_9;

    /// saturated float32 temp_10
    float temp_10;

    /// saturated float32 temp_11
    float temp_11;

    /// saturated float32 temp_12
    float temp_12;

    /// saturated float32 temp_13
    float temp_13;

    /// saturated float32 temp_14
    float temp_14;

    /// saturated float32 temp_15
    float temp_15;

    /// saturated float32 temp_16
    float temp_16;

    /// saturated float32 temp_17
    float temp_17;

    /// saturated float32 temp_18
    float temp_18;

    /// saturated float32 temp_19
    float temp_19;

    /// saturated float32 temp_20
    float temp_20;
} msg_dustbin_temperature_info_3_0;  //83   // 接收吸尘箱温度


typedef struct __attribute__((packed))
{
    /// saturated float32 current_1
    float current_1;

    /// saturated float32 current_2
    float current_2;

    /// saturated float32 current_3
    float current_3;

    /// saturated float32 current_4
    float current_4;

    /// saturated float32 current_5
    float current_5;

    /// saturated float32 current_6
    float current_6;

    /// saturated float32 current_7
    float current_7;

    /// saturated float32 current_8
    float current_8;

    /// saturated float32 current_9
    float current_9;

    /// saturated float32 current_10
    float current_10;

    /// saturated float32 current_11
    float current_11;

    /// saturated float32 current_12
    float current_12;

    /// saturated float32 current_13
    float current_13;

    /// saturated float32 current_14
    float current_14;

    /// saturated float32 current_15
    float current_15;

    /// saturated float32 current_16
    float current_16;

    /// saturated float32 current_17
    float current_17;

    /// saturated float32 current_18
    float current_18;

    /// saturated float32 current_19
    float current_19;

    /// saturated float32 current_20
    float current_20;
} msg_dustbin_current_info_3_0;  //85   //  接收吸尘箱电流
